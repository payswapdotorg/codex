...
- Avoid large modules:
  ...
  - When extracting code from a large module, move the related tests and module/type docs toward the
...
For complex logic changes the change should be under 500 lines.
...
- Prefer simple conversions: use "text".into() for spans, `vec![…].into()` for lines; when inference is ambiguous (e.g., Paragraph::new/Cell::from), use Line::from(spans) or Span::from(text).
...
- Use an explicit `#[path = "..._tests.rs"]` attribute so the test filename is descriptive:
...
**Requirement:** any change that affects user-visible UI (including adding new UI) must include corresponding `insta` snapshot coverage.

When UI or text output changes intentionally, update the snapshots as follows:

- Run tests to generate any updated snapshots:
  - `just test -p codex-tui`
- Check pending snapshots with `cargo insta pending-snapshots -p codex-tui`.
- Review `.snap.new` files and accept only intended updates.

### Benchmarks

Use `just bench`; smoke-test with `just bench-smoke` when introducing benchmarks.

### Test assertions

Tests should use `pretty_assertions::assert_eq` and prefer deep equality of whole objects. Avoid mutating process environment in tests.

### Spawning workspace binaries in tests (Cargo vs Bazel)

Prefer `codex_utils_cargo_bin::cargo_bin` over direct cargo binary discovery. Under Bazel, use runfile-safe resource resolution such as `codex_utils_cargo_bin::find_resource!` rather than `CARGO_MANIFEST_DIR`.

### Integration tests

Prefer `core_test_support::responses`, `TestCodexBuilder::build_with_auto_env()`, and `wait_for_event` for codex_core integration testing. App-server tests should exercise the public JSON-RPC API with `TestAppServer` and auto-env helpers.

## App-server API Development Best Practices

Active API development belongs in app-server v2, not v1. Use `*Params`, `*Response`, `*Notification`; singular `<resource>/<method>` names; camelCase wire fields/enums; `#[ts(export_to = "v2/")]`; explicit discriminated unions; string IDs at the API boundary; integer Unix-second timestamps; and `#[experimental]` gating where appropriate. Regenerate schemas with `just write-app-server-schema` and validate with `just test -p codex-app-server-protocol`.
...
Tests and features must support Linux, macOS and Windows unless explicitly OS-specific. Codex supports connected app-server and exec-server on different operating systems.