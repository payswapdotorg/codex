...
function uploadAsset(ctx) {
  ...
  const asset = {
    ...
    width: parseInt(b.width, 10) || 0,
    height: parseInt(b.height, 10) || 0,
    credit: String(b.credit || "Staff"),
    license: ["editorial", "handout", "purchased"].includes(b.license)
      ? b.license
      : "editorial",
    renditions: [],
  ...
  ctx.emit("asset.uploaded", {
    subject: asset.id,
    summary: `Asset ${asset.id} "${asset.name}" (${kind}, ${asset.sizeMb} MB) uploaded to the repository`,
...
function requestRendition(ctx) {
  ...
  if (!asset)
    throw new R.AppError(
      404,
      "asset_not_found",
      `Asset "${b.assetId}" does not exist.`,
    );
  ...
  if (ctx.failure === "data_conflict") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Rendition request conflicts with an in-flight job for ${asset.id} (simulated).`,
      { assetId: asset.id, simulated: true },
    );
  }
  const name = String(b.rendition || "thumbnail");
  if (
    asset.renditions.find((r) => r.name === name && r.status === "processing")
  ) {
    throw new R.AppError(
      409,
      "data_conflict",
      `Rendition "${name}" for ${asset.id} is already processing.`,
      { assetId: asset.id, rendition: name },
    );
  }
  const dimensions =
    name === "thumbnail"
  ...
  ctx.emit("rendition.ready", {
    subject: asset.id,
    summary: `Rendition "${name}" (${dimensions}) ready for asset ${asset.name}`,
    data: { assetId: asset.id, rendition: name },
  });
  return {
    message: `Rendition "${name}" rendered for ${asset.id}.`,
    redirect: "/assets",
    asset,
  };
}
...
function scheduleSocialPost(ctx) {
  ...
  const network = ["feedbook", "chirp", "linkup"].includes(b.network)
    ? b.network
    : "feedbook";
  if (
    ctx.state.socialPosts.find(
      (p) =>
        p.storyId === story.id &&
        p.network === network &&
        ["scheduled", "posted"].includes(p.status),
    )
  ) {
    throw new R.AppError(
      409,
  ...
  const post = {
    id: R.nextId(ctx.state, "socialSeq", "sp-"),
    storyId: story.id,
    network,
    text: String(b.text || story.title),
    status: "scheduled",
  ...
  ctx.emit("social.scheduled", {
    subject: post.id,
    summary: `${network} post scheduled for "${story.title}" (${post.scheduledAt}) by ${ctx.user.name}`,
    data: {
      postId: post.id,
      storyId: story.id,
      network,
  ...
  const n = ctx.notify(
    "u-1",
    "social",
    `${network} post scheduled for "${story.title}"`,
  ...
  return {
    ...
    warning:
      n && n.status === "failed"
        ? "Post scheduled, but the editor notification could not be delivered."
        : null,
    redirect: "/social",
    post,
  };
...
function publishSocialPost(ctx) {
  ...
  if (!post)
    throw new R.AppError(
      404,
      "post_not_found",
      `Social post "${b.postId}" does not exist.`,
    );
  if (ctx.failure === "data_conflict") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Post ${post.id} was already published by someone else (simulated).`,
      { postId: post.id, simulated: true },
    );
  }
  if (post.status === "posted") {
    throw new R.AppError(
      409,
      "duplicate_event",
      `Post ${post.id} was already published.`,
      { postId: post.id },
    );
  }
  ...
  if (failed) {
    ...
    ctx.emit("social.post_failed", {
      subject: post.id,
      summary: `${post.network} post for story ${post.storyId} FAILED to deliver (simulated platform outage)`,
  ...
  ctx.emit("social.posted", {
    subject: post.id,
    summary: `${post.network} post published for story ${post.storyId} by ${ctx.user.name}`,
    data: { postId: post.id, network: post.network },
  });
  return {
    message: `Post ${post.id} published to ${post.network}.`,
    redirect: "/social",
    post,
  };
}
