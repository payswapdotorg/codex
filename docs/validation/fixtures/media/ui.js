...
function dashboard(ctx) {
  ...
  const failedNotifs = ctx.user
    ? s.notifications.filter(
        (n) => n.userId === ctx.user.id && n.status === "failed",
      ).length
    : 0;
  const storiesTable = table(
    ["Story", "Section", "Author", "Status", "Version", "Published"],
    s.stories,
    (st) =>
      W.tr([
        `<a href="/stories/${esc(st.id)}"><b>${esc(st.title)}</b></a>`,
        esc(st.section),
        esc(userName(s, st.authorId)),
        W.statusPill(st.status),
        `v${st.version}`,
        st.publishedAt
          ? `<span class="mono">${esc(st.publishedAt)}</span>`
          : "—",
      ]),
  );
...
function storyPage(ctx) {
  ...
  const canEdit =
    ctx.user &&
    (isAuthor || isEditor) &&
    ["draft", "changes_requested", "in_review"].includes(story.status);
  const canSubmit =
    isAuthor && ["draft", "changes_requested"].includes(story.status);
  const canDecide = isEditor && story.status === "in_review" && !isAuthor;
  const canPublish =
    ctx.user &&
    ctx.user.permissions.includes("story:publish") &&
    story.status === "approved";
  const canCorrect =
    isEditor && ["published", "corrected"].includes(story.status);
  const jobs = s.distributionJobs
    .filter((j) => j.storyId === story.id)
    .slice()
    .reverse();
  const posts = story.socialPosts
    .map((pid) => s.socialPosts.find((p) => p.id === pid))
    .filter(Boolean);
  ...
  const overview = W.dl([
    ...
    ["Slug", `<code>${esc(story.slug)}</code>`],
    ["Section", esc(story.section)],
    ["Author", esc(userName(s, story.authorId))],
    ["Status", W.statusPill(story.status)],
    ["Version", `v${story.version}`],
    ["Word count", story.wordCount],
    [
      "Approved by",
      story.approvedById ? esc(userName(s, story.approvedById)) : "—",
    ],
    [
      "Published at",
      story.publishedAt
        ? `<span class="mono">${esc(story.publishedAt)}</span>`
        : "—",
    ],
    [
      "Channels",
      story.channels
        .map((c) => `<code>${esc(channelName(s, c))}</code>`)
        .join(", ") || "—",
    ],
  ]);
  const assetsTable = story.assets.length
    ? table(
        ["Asset", "Kind", "Name", "Credit", "Renditions"],
        story.assets,
        (a) => {
          const asset = s.assets.find((x) => x.id === a.assetId) || {};
          return W.tr([
            `<code>${esc(a.assetId)}</code> ${W.pill("info", a.role)}`,
            esc(asset.kind || "?"),
            esc(asset.name || "?"),
            esc(asset.credit || ""),
            (asset.renditions || [])
              .map((r) => `${esc(r.name)}:${W.statusPill(r.status)}`)
              .join(" "),
          ]);
        },
      )
      ...
      ? W.frm(
          ...
          [
            ...
            W.fld(
              ...
              W.sel(
                "assetId",
                s.assets.map((a) => [a.id, `${a.id} — ${a.name} (${a.kind})`]),
              ),
            ),
            W.fld(
              "Role",
              W.sel("role", [
                ["hero", "Hero (required to publish)"],
                ["inline", "Inline"],
                ["video", "Video"],
              ]),
            ),
          ...
          {
            submit: "Attach asset",
            note: "missing_asset: attaching an unknown id fails with 404.",
          },
        )
      : "";
  const editForm = canEdit
    ? W.frm(
        ...
        ],
        {
          ...
          note: "Optimistic concurrency: saving a stale version returns data_conflict.",
        },
      )
    : "";
  const submitForm = canSubmit
    ? W.frm("/ui/stories/submit", [W.hidden("storyId", story.id)], {
  ...
  const publishForm = canPublish
    ? W.frm(
        ...
        [
          ...
          W.fld(
            ...
            W.checks(
              ...
              s.channels.map((c) => [
                c.id,
                `${c.name}${isPast(ctx, c.licenseValidUntil) ? " ⚠ license expired" : ""}`,
                c.id === "ch-1",
              ]),
            ),
          ),
        ...
        {
          submit: "Publish",
          note: `Requires a hero image. Publish ships v${story.version} (expectedVersion) — a stale version is rejected with data_conflict (reload and re-review). Channels with expired licenses fail with stale_entitlement (seed: partner-app).`,
        },
      )
    : "";
  const correctForm = canCorrect
    ? W.frm(
        "/ui/stories/correct",
        [
          W.hidden("storyId", story.id),
        ...
        ],
        {
          ...
          note: "Applies the correction, bumps the version and re-distributes to the story channels.",
        },
      )
    : "";
  const jobsTable = jobs.length
    ? table(["Job", "Channel", "Status", "At", "Error"], jobs, (j) =>
        W.tr([
          `<code>${esc(j.id)}</code>`,
          `<code>${esc(channelName(s, j.channelId))}</code>`,
          W.statusPill(j.status),
          `<span class="mono">${esc(j.at)}</span>`,
          j.error ? `<span class="muted">${esc(j.error)}</span>` : "—",
        ]),
      )
    : '<p class="muted">No distribution jobs.</p>';
  const postsTable = posts.length
    ? table(["Post", "Network", "Text", "Status", "Engagement"], posts, (p) =>
        W.tr([
          `<code>${esc(p.id)}</code>`,
          esc(p.network),
          esc(p.text),
          W.statusPill(p.status),
          p.engagement
            ? `${p.engagement.likes} likes · ${p.engagement.reposts} reposts · ${p.engagement.clicks} clicks`
            : "—",
        ]),
      )
    : '<p class="muted">No social posts.</p>';
  const corrTable = corrs.length
    ? table(
        ["Correction", "Summary", "Before", "After", "By", "At"],
        corrs,
        (c) =>
          W.tr([
            `<code>${esc(c.id)}</code>`,
            esc(c.summary),
            `<code>${esc(c.beforeText)}</code>`,
            `<code>${esc(c.afterText)}</code>`,
            esc(userName(s, c.byId)),
            `<span class="mono">${esc(c.at)}</span>`,
          ]),
      )
    : '<p class="muted">No corrections.</p>';
  const socialForm =
    can(ctx, "social:schedule") &&
...
function isPast(ctx, d) {
  return String(d || "9999-12-31") < (ctx.state.meta.today || "2026-09-12");
}
    ...
    ? W.card(
        ...
        W.frm(
          ...
          [
            ...
            W.fld(
              "File name",
              W.txt("name", "city-hall-presser.jpg", { required: true }),
            ),
            W.fld(
              "Size (MB)",
              W.txt("sizeMb", "2.5", {
...
function social(ctx) {
  ...
  const rows = s.socialPosts
    .slice()
    .reverse()
    .map((p) =>
      W.tr([
        `<code>${esc(p.id)}</code>`,
        storyLink(s, p.storyId),
        esc(p.network),
        esc(p.text),
        W.statusPill(p.status),
        `<span class="mono">${esc(p.scheduledAt)}</span>`,
        p.engagement
          ? `${p.engagement.likes} / ${p.engagement.reposts} / ${p.engagement.clicks}`
          : "—",
        canPost && p.status === "scheduled"
          ? W.frm("/ui/social/publish", [W.hidden("postId", p.id)], {
              submit: "Post now",
            })
          : "—",
      ]),
    ...
    .join("");
  const scheduleForm = canSchedule
    ? W.card(
          ...
          [
            W.fld(
              ...
              W.sel(
                ...
                s.stories
                  .filter((st) =>
                    ["published", "corrected"].includes(st.status),
                  )
  ...
  const rows = s.channels
    .map((c) =>
      W.tr([
        ...
        esc(c.kind),
        c.licenseValidUntil
          ? isPast(ctx, c.licenseValidUntil)
            ? `${esc(c.licenseValidUntil)} ${W.pill("err", "expired")}`
            : `${esc(c.licenseValidUntil)} ${W.pill("ok", "valid")}`
