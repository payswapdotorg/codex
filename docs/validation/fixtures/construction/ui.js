...
function findVendor(s, id) {
  return (s.contractors || []).find((c) => c.id === id);
}
...
function dashboard(ctx) {
  const s = ctx.state;
  const openPRs = s.purchaseRequests.filter(
    (r) => r.status === "submitted",
  ).length;
  const openInc = s.safetyIncidents.filter((i) => i.status !== "closed").length;
  const myNotifs = ctx.user
    ? s.notifications.filter((n) => n.userId === ctx.user.id)
    : [];
  ...
  const projectsTable = table(
    ["Code", "Project", "Phase", "Progress", "Spend / budget", "Due"],
    s.projects,
    (p) =>
      W.tr([
        `<code>${esc(p.code)}</code>`,
        `<a href="/projects/${esc(p.id)}">${esc(p.name)}</a>`,
        esc(p.phase),
        W.progress(p.percentComplete) +
          ` <span class="muted">${p.percentComplete}%</span>`,
        `${W.money(p.spentUsd)} / ${W.money(p.budgetUsd)}`,
        esc(p.dueDate),
      ]),
  );
  const eventsTable = W.tbl(
    ["Time", "Type", "Summary"],
    recent
      .map((e) =>
        W.tr([
      ...
      .join("") +
      `<tr><td colspan="3"><a href="/events">Full event feed</a> · machine: <code>GET /api/events</code></td></tr>`,
  );
...
function projects(ctx) {
  const s = ctx.state;
  const t = table(
    ["Code", "Project", "Site", "Phase", "Progress", "Manager", "Contractors"],
    s.projects,
    (p) =>
      W.tr([
        `<code>${esc(p.code)}</code>`,
        ...
        W.progress(p.percentComplete) +
          ` <span class="muted">${p.percentComplete}%</span>`,
        esc(userName(s, p.managerId)),
        p.contractorIds.map((c) => esc(vendorName(s, c))).join(", "),
      ]),
  );
  return W.page(ctx.app.config, {
    title: "Projects",
...
function projectPage(ctx) {
  ...
  if (!detail)
    throw new R.AppError(
      404,
      "project_not_found",
      `Project "${ctx.params.id}" does not exist.`,
    );
  const p = detail.project;
  const canReport =
    ctx.user && ctx.user.permissions.includes("progress:create");
  const photoOptions = detail.photos.map((a) => [a.id, `${a.id} — ${a.name}`]);
  const taskOptions = detail.tasks
    .filter((t) => t.status !== "done")
    .map((t) => [t.id, `${t.id} — ${t.title}`]);
  const overview = W.dl([
    ["Site", esc(p.site)],
    ["Phase", esc(p.phase)],
    ["Progress", W.progress(p.percentComplete) + ` ${p.percentComplete}%`],
    ["Budget", `${W.money(p.spentUsd)} of ${W.money(p.budgetUsd)}`],
    ["Manager", esc(userName(s, p.managerId))],
    [
      "Contractors",
      p.contractorIds
        .map(
          (c) =>
            `${esc(vendorName(s, c))} <span class="muted">until ${esc((findVendor(s, c) || {}).contractValidUntil || "?")}</span>`,
        )
        .join("<br>"),
    ],
    ["Due", esc(p.dueDate)],
  ...
  const tasksTable = table(
    ["Task", "Trade", "Status", "Assignee"],
    detail.tasks,
    (t) =>
      W.tr([
        `<code>${esc(t.id)}</code> ${esc(t.title)}`,
        esc(t.trade),
        W.statusPill(t.status),
        esc(t.assigneeId ? userName(s, t.assigneeId) : "—"),
      ]),
  );
  const photosTable = detail.photos.length
    ? table(["Asset", "Name", "Caption", "Size"], detail.photos, (a) =>
        W.tr([
          `<code>${esc(a.id)}</code>`,
          esc(a.name),
          esc(a.caption),
  ...
  const reportForm = canReport
    ? W.card(
        "Submit daily progress report (foreman)",
        W.frm(
          "/ui/progress",
          [
            ...
            taskOptions.length
              ? W.fld(
                  "Tasks completed",
                  W.sel("taskIds", taskOptions, {
                    multi: true,
                    size: Math.min(6, taskOptions.length),
                  }),
            ...
            W.fld(
              "Percent complete",
              W.txt("percentComplete", p.percentComplete, {
                type: "number",
                min: 0,
                max: 100,
              }),
            ...
            photoOptions.length
              ? W.fld(
                  "Photos attached",
                  W.sel("photoIds", photoOptions, {
                    multi: true,
                    size: Math.min(4, photoOptions.length),
                  }),
                  "Optional; pick site photos",
                )
              : "",
          ...
          ],
          {
            ...
            note: "Creates the report, marks tasks done, updates project %, notifies the PM. The API twin POST /api/progress accepts the same fields plus ?failure= switches.",
          },
        ),
      )
    : "";
  const uploadForm = canReport
    ? W.card(
        "Upload site photo",
        W.frm(
          "/ui/assets/upload",
          [
            ...
            W.fld(
              "File name",
              W.txt("name", "level13-formwork.jpg", { required: true }),
            ),
            W.fld("Caption", W.txt("caption", "")),
            W.fld(
              "Size (kb)",
              W.txt("sizeKb", "1500", { type: "number", min: 1 }),
            ),
          ],
          {
            submit: "Upload photo",
            note: "Simulated upload — records the asset and emits asset.uploaded.",
          },
        ),
      )
    : "";
  const reportsTable = detail.reports.length
    ? table(
        ["Report", "Date", "By", "%", "Tasks", "Photos", "Notes"],
        detail.reports.slice().reverse(),
        (r) =>
          W.tr([
            `<code>${esc(r.id)}</code>`,
            esc(r.reportDate),
            esc(userName(s, r.authorId)),
            `${r.percentComplete}%`,
            esc(r.taskIds.join(", ") || "—"),
            esc(r.photoIds.join(", ") || "—"),
            esc(r.notes),
  ...
  const incidentsTable = detail.incidents.length
    ? table(
  ...
  const docsTable = detail.documents.length
    ? table(["Document", "Version", "Owner", "Asset"], detail.documents, (d) =>
        W.tr([
          esc(d.name),
          "v" + d.version,
          esc(userName(s, d.owner)),
          d.assetId
            ? `<code>${esc(d.assetId)}</code>`
...
function procurement(ctx) {
  ...
  const canInvoice =
    ctx.user && ctx.user.permissions.includes("invoice:approve");
  const prRows = s.purchaseRequests
    .map((r) =>
      W.tr([
        `<code>${esc(r.id)}</code>`,
        esc(r.item),
        `${r.qty} ${esc(r.unit)}`,
        W.money(r.totalUsd),
        esc(vendorName(s, r.vendorId)),
        `<span class="muted">until ${esc((findVendor(s, r.vendorId) || {}).contractValidUntil || "?")}</span>`,
        `<code>${esc(r.projectId)}</code>`,
        W.statusPill(r.status),
        canApprove && (r.status === "submitted" || r.status === "screening")
          ? W.frm(
              "/ui/procurement/decide",
              [
                W.hidden("requestId", r.id),
                W.fld(
                  "Decision",
                  W.sel("decision", [
                    ["approve", "Approve (issue PO)"],
                    ["reject", "Reject"],
                  ]),
                ),
          ...
          : r.approvals
              .map((a) => `${esc(a.decision)} by ${esc(userName(s, a.byId))}`)
              .join("; ") || "—",
      ]),
    )
    .join("");
  const poRows = s.purchaseOrders
    .map((o) =>
      W.tr([
        `<code>${esc(o.id)}</code>`,
        esc(vendorName(s, o.vendorId)),
        W.money(o.totalUsd),
        W.statusPill(o.status),
        esc(userName(s, o.issuedById)),
      ]),
    )
    .join("");
  const invRows = s.invoices
    .map((v) =>
      W.tr([
        `<code>${esc(v.id)}</code>`,
        esc(vendorName(s, v.vendorId)),
        `<code>${esc(v.poNumber || "—")}</code>`,
        W.money(v.amountUsd),
        esc(v.dueDate || "—"),
        W.statusPill(v.status),
        canInvoice && v.status === "received"
          ? W.frm("/ui/invoices/approve", [W.hidden("invoiceId", v.id)], {
              submit: "Approve for payment",
            })
          : v.approvedById
            ? esc(userName(s, v.approvedById))
            : "—",
      ]),
    )
    .join("");
...
${W.card(
  "Purchase requests" + (canApprove ? " — decide as project manager" : ""),
  W.tbl(
    [
      "PR",
      "Item",
...
${W.card(
  "Invoices" + (canInvoice ? " — approve as finance" : ""),
  W.tbl(
    [
      "Invoice",
      "Vendor",
...
function safety(ctx) {
  ...
  const rows = s.safetyIncidents
    .slice()
    .reverse()
    .map((i) =>
      W.tr([
        `<code>${esc(i.id)}</code>`,
        `<code>${esc(i.projectId)}</code>`,
        W.statusPill(i.severity),
        esc(i.category),
        W.statusPill(i.status),
        esc(i.description),
        esc(userName(s, i.reportedById)),
        canClose && i.status !== "closed"
          ? W.frm(
              "/ui/safety/close",
              [
                W.hidden("incidentId", i.id),
                W.fld("Resolution", W.txt("resolution", "")),
              ],
        ...
        W.frm(
          ...
          [
            W.fld(
              ...
              W.sel(
                "projectId",
                s.projects.map((p) => [p.id, `${p.code} — ${p.name}`]),
              ),
            ),
            W.fld(
              "Severity",
            ...
            W.fld(
              "Description",
              W.ta("description", "What happened, where, who was involved."),
            ),
          ],
          {
            submit: "Report incident",
            note: "Records the incident and notifies the safety officer.",
          },
        ),
      )
    : "";
  ...
  const rows = mine
    .map((n) =>
      W.tr([
        ...
        `<span class="mono">${esc(n.createdAt)}</span>`,
      ]),
    )
    .join("");
  return W.page(ctx.app.config, {
