...
function isPast(ctx, dateStr) {
  return String(dateStr || "") < today(ctx);
}
...
function submitProgress(ctx) {
  ...
  if (!project)
    throw new R.AppError(
      404,
      "project_not_found",
      `Project "${b.projectId}" does not exist.`,
    );
  const date = String(b.reportDate || today(ctx)).slice(0, 10);
  if (
    ctx.state.progressReports.find(
      (r) => r.projectId === project.id && r.reportDate === date,
    )
  ) {
    throw new R.AppError(
      409,
  ...
  const taskIds = R.asList(b.taskIds).filter(
    (t) =>
      find(ctx.state.tasks, t) &&
      find(ctx.state.tasks, t).projectId === project.id,
  );
  ...
  for (const pid of photoIds) {
    ...
    if (!a || a.kind !== "photo")
      throw new R.AppError(
        404,
        "missing_asset",
        `Photo asset "${pid}" does not exist in the photo store.`,
        { assetId: pid },
      );
  }
  const report = {
    ...
    percentComplete: Math.max(
      0,
      Math.min(100, parseInt(b.percentComplete, 10) || project.percentComplete),
    ),
  ...
  for (const t of taskIds) {
    const task = find(ctx.state.tasks, t);
    if (task) task.status = "done";
  }
  if (report.percentComplete > project.percentComplete)
  ...
  const n = ctx.notify(
    project.managerId,
    "progress",
    `Daily progress ${report.reportDate} — ${project.name}`,
  ...
  return {
    ...
    warning:
      n && n.status === "failed"
        ? `Saved, but the notification to the project manager could not be delivered (check /notifications and the event feed for notification.failed).`
        : null,
...
function uploadPhoto(ctx) {
  ...
  const asset = {
    ...
    projectId: project.id,
    kind: "photo",
    name: String(b.name || "untitled.jpg"),
    sizeKb: parseInt(b.sizeKb, 10) || 900,
  ...
  ctx.emit("asset.uploaded", {
    subject: asset.id,
    summary: `Photo "${asset.name}" attached to ${project.name} (${asset.sizeKb} kb)`,
    data: { assetId: asset.id, projectId: project.id },
  });
  return {
    message: `Photo ${asset.id} ("${asset.name}") stored for ${project.name}.`,
    redirect: `/projects/${project.id}`,
    asset,
  };
}
...
function decidePurchaseRequest(ctx) {
  ...
  if (!pr)
    throw new R.AppError(
      404,
      "pr_not_found",
      `Purchase request "${b.requestId}" does not exist.`,
    );
  ...
  if (decision === "approve") {
    if (vendor && isPast(ctx, vendor.contractValidUntil)) {
      throw new R.AppError(
        ...
        `Vendor contract for ${vendor.company} expired on ${vendor.contractValidUntil} — a new vendor agreement must be in place before this request can be approved.`,
        {
          ...
          contractValidUntil: vendor.contractValidUntil,
        },
      );
    }
    const po = {
      id: R.nextId(ctx.state, "poSeq", "po-"),
      requestId: pr.id,
      vendorId: pr.vendorId,
    ...
    ctx.emit("procurement.approved", {
      subject: pr.id,
      summary: `PR ${pr.id} (${pr.item}) approved by ${ctx.user.name}`,
      data: { requestId: pr.id, totalUsd: pr.totalUsd, vendorId: pr.vendorId },
    });
    ctx.emit("po.issued", {
      subject: po.id,
      summary: `Purchase order ${po.id} issued to ${vendor ? vendor.company : pr.vendorId} for $${pr.totalUsd.toLocaleString("en-US")}`,
      data: { poId: po.id, requestId: pr.id, totalUsd: pr.totalUsd },
    });
    ctx.notify(
      pr.requestedById,
      "procurement",
      `PR ${pr.id} approved — PO ${po.id} issued`,
      `${ctx.user.name} approved "${pr.item}". PO ${po.id} was issued to the vendor.`,
    );
    const n2 = ctx.notify(
      "u-5",
      "procurement",
      `PO ${po.id} issued to your company`,
    ...
    return {
      ...
      warning:
        n2 && n2.status === "failed"
          ? "PO issued, but the vendor notification could not be delivered."
          : null,
  ...
  ctx.emit("procurement.rejected", {
    subject: pr.id,
    summary: `PR ${pr.id} (${pr.item}) rejected by ${ctx.user.name}`,
  ...
  ctx.notify(
    ...
    "procurement",
    `PR ${pr.id} rejected`,
    `${ctx.user.name} rejected "${pr.item}". Note: ${b.note || "n/a"}`,
  );
  return {
    message: `PR ${pr.id} rejected.`,
    redirect: "/procurement",
    purchaseRequest: pr,
  };
}
...
function approveInvoice(ctx) {
  ...
  if (!inv)
    throw new R.AppError(
      404,
      "invoice_not_found",
      `Invoice "${b.invoiceId}" does not exist.`,
    );
  if (ctx.failure === "data_conflict") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Invoice ${inv.id} was already processed by another accountant (simulated conflict).`,
      { invoiceId: inv.id, currentStatus: inv.status, simulated: true },
    );
  }
  if (inv.status !== "received") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Invoice ${inv.id} is already "${inv.status}".`,
      { invoiceId: inv.id, currentStatus: inv.status },
    );
  }
  inv.status = "approved";
  inv.approvedById = ctx.user.id;
  inv.at = new Date().toISOString();
  ctx.emit("invoice.approved", {
    subject: inv.id,
    summary: `Invoice ${inv.id} ($${inv.amountUsd.toLocaleString("en-US")}) approved for payment by ${ctx.user.name}`,
    data: {
      invoiceId: inv.id,
  ...
  ctx.notify(
    "u-1",
    "finance",
    `Invoice ${inv.id} approved`,
    `${ctx.user.name} approved invoice ${inv.id} for payment.`,
  );
  return {
    message: `Invoice ${inv.id} approved for payment.`,
    redirect: "/procurement",
    invoice: inv,
  };
}
...
function reportSafetyIncident(ctx) {
  ...
  const inc = {
    id: R.nextId(ctx.state, "safetySeq", "si-"),
    projectId: project.id,
    severity: ["minor", "moderate", "major"].includes(b.severity)
      ? b.severity
      : "minor",
  ...
  ctx.emit("safety.incident_reported", {
    subject: inc.id,
    summary: `Safety incident (${inc.severity}/${inc.category}) reported on ${project.name}`,
    data: {
      incidentId: inc.id,
      projectId: project.id,
  ...
  const n = ctx.notify(
    "u-4",
    "safety",
    `New ${inc.severity} incident on ${project.name}`,
    `${ctx.user.name} reported: ${inc.description}`,
  );
  return {
    message: `Safety incident ${inc.id} recorded; the safety officer was notified.`,
    warning:
      n && n.status === "failed"
        ? "Incident saved, but the notification to the safety officer could not be delivered."
        : null,
    redirect: "/safety",
    incident: inc,
  };
...
function closeSafetyIncident(ctx) {
  ...
  if (!inc)
    throw new R.AppError(
      404,
      "incident_not_found",
      `Safety incident "${b.incidentId}" does not exist.`,
    );
  if (ctx.failure === "data_conflict") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Incident ${inc.id} was already closed by another officer (simulated conflict).`,
      { incidentId: inc.id, simulated: true },
    );
  }
  if (inc.status === "closed")
    throw new R.AppError(
      409,
      "data_conflict",
      `Incident ${inc.id} is already closed.`,
      { incidentId: inc.id },
    );
  inc.status = "closed";
  inc.actions.push({
    byId: ctx.user.id,
    text: String(b.resolution || "Closed."),
    at: new Date().toISOString(),
  });
  ctx.emit("safety.incident_closed", {
    subject: inc.id,
    summary: `Safety incident ${inc.id} closed by ${ctx.user.name}`,
    data: { incidentId: inc.id, projectId: inc.projectId },
  });
  ctx.notify(
    inc.reportedById,
    "safety",
    `Incident ${inc.id} closed`,
    `${ctx.user.name} closed incident ${inc.id}: ${b.resolution || ""}`,
  );
  return {
    message: `Safety incident ${inc.id} closed.`,
    redirect: "/safety",
    incident: inc,
  };
}
...
function projectDetail(state, id) {
  ...
  return {
    ...
    photos: state.assets.filter(
      (a) => a.projectId === id && a.kind === "photo",
    ),
    contractors: state.contractors.filter((c) =>
      p.contractorIds.includes(c.id),
    ),
  };
}
