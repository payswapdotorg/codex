...
function dashboard(ctx) {
  ...
  const servicesTable = table(
    ["Service", "Tier", "Status", "Version", "Uptime 30d", "p95", "Owner"],
    s.services,
    (v) =>
      W.tr([
        `<b>${esc(v.name)}</b>`,
        ...
        `${v.uptime30d}%`,
        `${v.latencyMs} ms`,
        esc(teamName(s, v.ownerTeamId)),
      ]),
  );
  const prsTable = table(
    ["PR", "Repo", "Title", "Author", "CI", "Approvals", "Status"],
    openPRs,
    (p) =>
      W.tr([
        `<a href="/prs/${esc(p.id)}"><code>${esc(p.id)}</code> #${p.number}</a>`,
        `<code>${esc(repoName(s, p.repoId))}</code>`,
        esc(p.title),
        esc(userName(s, p.authorId)),
        W.statusPill(latestCI(s, p).status),
        `${p.approvals.length}`,
        W.statusPill(p.status),
      ]),
  );
  const incTable = table(
    ["Incident", "Severity", "Title", "Service", "Status"],
    openInc,
    (i) =>
      W.tr([
        `<code>${esc(i.id)}</code>`,
        ...
        esc(i.title),
        `<code>${esc(svcName(s, i.serviceId))}</code>`,
...
function repos(ctx) {
  ...
  const t = table(
    ...
    (r) =>
      W.tr([
        `<a href="/repos/${esc(r.id)}"><b>${esc(r.name)}</b></a>`,
        esc(r.language),
        esc(teamName(s, r.ownerTeamId)),
        r.reviewerIds.map((u) => esc(userName(s, u))).join(", "),
        `<code>${esc(r.currentStagingVersion)}</code>`,
        `<code>${esc(r.productionVersion)}</code>`,
        String(
          s.pullRequests.filter((p) => p.repoId === r.id && p.status === "open")
            .length,
        ),
      ]),
  );
...
function repoPage(ctx) {
  ...
  const deps = s.deployments
    .filter((d) => d.repoId === repo.id)
    .slice()
    .reverse();
  const arts = s.artifacts.filter((a) => a.repoId === repo.id);
  const createForm = can(ctx, "pr:create")
    ? W.card(
        "Open a pull request",
        W.frm(
          "/ui/prs/create",
          [
            ...
            W.fld(
              ...
              W.txt("title", "", {
                required: true,
                placeholder: "Refund idempotency keys",
              }),
            ),
            W.fld(
              "Source branch",
              W.txt(
                "branch",
                (ctx.user ? ctx.user.username.split(".")[0] : "user") +
                  "/feature",
                { required: true },
              ),
            ),
            ...
            W.fld(
              "Additions / deletions",
              W.txt("additions", "120") + W.txt("deletions", "18"),
            ),
            W.fld("Summary", W.ta("summary", "What does this change do?")),
          ],
          {
            submit: "Open PR",
            note: "Opening a PR automatically runs CI (lint, unit, integration) and requests review.",
          },
        ),
      )
    : "";
  const prsTable = table(
    ["PR", "Title", "Author", "CI", "Approvals", "Status"],
    prs,
    (p) =>
      W.tr([
        `<a href="/prs/${esc(p.id)}"><code>${esc(p.id)}</code> #${p.number}</a>`,
  ...
  const depsTable = table(
    ["Deployment", "PR", "Env", "Version", "Artifact", "Status", "Action"],
    deps,
    (d) =>
      W.tr([
        `<code>${esc(d.id)}</code>`,
        d.prId ? `<code>${esc(d.prId)}</code>` : "—",
        W.statusPill(
          d.env === "production"
            ? "info"
            : d.status === "succeeded"
        ...
        `<code>${esc(d.version)}</code>`,
        `<code>${esc(d.artifactId)}</code>`,
        W.statusPill(d.status),
        promoteForm(ctx, d) || rollbackForm(ctx, d) || "—",
      ]),
  );
  const artsTable = arts.length
    ? table(["Artifact", "Digest", "Size", "Built from"], arts, (a) =>
        W.tr([
          `<code>${esc(a.id)}</code>`,
          `<code>${esc(a.digest)}</code>`,
          `${(a.sizeKb / 1024).toFixed(1)} MB`,
          `<code>${esc(a.builtFromPr)}</code>`,
        ]),
      )
...
function promoteForm(ctx, d) {
  if (
    !can(ctx, "deploy:promote") ||
    d.env !== "staging" ||
    d.status !== "succeeded"
  )
    return "";
  return W.frm("/ui/deployments/promote", [W.hidden("deploymentId", d.id)], {
...
function rollbackForm(ctx, d) {
  if (
    !can(ctx, "deploy:rollback") ||
    d.env !== "production" ||
    d.status !== "succeeded"
  )
    return "";
  return W.frm("/ui/deployments/rollback", [W.hidden("deploymentId", d.id)], {
    submit: "Rollback",
  });
}
...
function prPage(ctx) {
  ...
  const runs = pr.ciRuns
    .map((rid) => s.ciRuns.find((r) => r.id === rid))
    .filter(Boolean)
    .reverse();
  ...
  const approveForm = canApprove
    ? W.card(
        ...
        W.frm(
          ...
          {
            submit: "Approve",
            note: "Self-approval is blocked by policy — the author cannot approve their own PR.",
          },
        ),
      )
    : "";
  const mergeForm = canMerge
    ? W.card(
        "Merge (release manager / code owner)",
        W.frm("/ui/prs/merge", [W.hidden("prId", pr.id)], {
          submit: "Merge & deploy to staging",
          note: "Requires ≥1 approval and a passing CI run. Merging deploys the version to staging and creates a build artifact.",
        }),
      )
    : "";
  const rerunForm = canRerun
    ? W.card(
        "Re-run CI",
        W.frm("/ui/prs/ci", [W.hidden("prId", pr.id)], {
          submit: "Re-run CI",
          sec: true,
        }),
      )
  ...
  const runsHtml = runs.length
    ? runs
        .map((r) =>
          W.card(
            `CI run <code>${esc(r.id)}</code> — ${W.statusPill(r.status)}`,
            table(["Step", "Status", "Duration"], r.steps, (st) =>
              W.tr([
                esc(st.name),
...
${W.card(
  ...
  W.dl([
    [
      "Repo",
      `<a href="/repos/${esc(repo.id)}"><code>${esc(repo.name)}</code></a>`,
    ],
