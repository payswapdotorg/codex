...
function deployments(ctx) {
  ...
  const rows = deps
    .map((d) =>
      W.tr([
        `<code>${esc(d.id)}</code>`,
        `<code>${esc(repoName(s, d.repoId))}</code>`,
        d.prId ? `<code>${esc(d.prId)}</code>` : "—",
        W.statusPill(d.env === "production" ? "info" : "neutral") + ` ${d.env}`,
        `<code>${esc(d.version)}</code>`,
        `<code>${esc(d.artifactId)}</code>`,
        W.statusPill(d.status),
        d.promotedById
          ? esc(userName(s, d.promotedById))
          : d.rollbackOf
            ? `rollback of <code>${esc(d.rollbackOf)}</code>`
            : "—",
        d.env === "staging" &&
        d.status === "succeeded" &&
        can(ctx, "deploy:promote")
          ? W.frm("/ui/deployments/promote", [W.hidden("deploymentId", d.id)], {
        ...
        can(ctx, "deploy:rollback")
          ? W.frm(
              "/ui/deployments/rollback",
              [W.hidden("deploymentId", d.id)],
              { submit: "Rollback", sec: true },
            )
          : "",
      ]),
    )
        ...
        canResolve && i.status !== "resolved"
          ? W.frm(
              ...
              [
                ...
                W.fld(
                  "Resolution",
                  W.txt("resolution", "Mitigated and verified."),
                ),
              ],
              { submit: "Resolve" },
            )
        ...
        W.frm(
          ...
          [
            W.fld(
              ...
              W.sel(
                "serviceId",
                s.services.map((v) => [v.id, `${v.name} (T${v.tier})`]),
              ),
            ),
            W.fld(
              "Severity",
            ...
            W.fld(
              ...
              W.ta(
                "description",
                "Customer impact, blast radius, first suspicions.",
              ),
            ),
          ],
          {
            submit: "Open incident",
            note: "Marks the service degraded/down and pages on-call.",
          },
        ),
      )
    : "";
  ...
  const t = table(
    ...
    (v) =>
      W.tr([
        ...
        v.dependsOn
          .map((d) => `<code>${esc(svcName(s, d))}</code>`)
          .join(", ") || "—",
      ]),
  );
  return W.page(ctx.app.config, {
...
function issues(ctx) {
  ...
  const rows = s.issues
    .map((i) =>
      W.tr([
        `<code>${esc(i.id)}</code>`,
        `<code>${esc(repoName(s, i.repoId))}</code>`,
        esc(i.title),
        W.statusPill(i.type),
        W.statusPill(i.priority),
        W.statusPill(i.status),
        i.assigneeId ? esc(userName(s, i.assigneeId)) : "—",
        i.labels.map((l) => W.pill("info", l)).join(" "),
        canClose && i.status !== "closed"
          ? W.frm("/ui/issues/close", [W.hidden("issueId", i.id)], {
              submit: "Close",
              sec: true,
            })
          : "—",
...
function docs(ctx) {
  const s = ctx.state;
  const t = table(["Doc", "Path", "Owner team", "Version"], s.docs, (d) =>
    W.tr([
      `<a href="/docs/${esc(d.id)}"><b>${esc(d.title)}</b></a>`,
      `<code>${esc(d.path)}</code>`,
      esc(teamName(s, d.ownerTeamId)),
      `v${d.version}`,
    ]),
  );
  return W.page(ctx.app.config, {
...
function docPage(ctx) {
  ...
  if (!doc)
    throw new R.AppError(
      404,
      "doc_not_found",
      `Document "${ctx.params.id}" does not exist.`,
    );
    ...
    ? W.card(
        ...
        W.frm(
          ...
          [
            ...
            W.fld(
              "Body (v" + doc.version + " → v" + (doc.version + 1) + ")",
              W.ta("body", doc.body, 8),
            ),
          ],
          {
      ...
      W.card(
        ...
        W.dl([
          ...
          [
            "Owns repos",
            t.ownsRepos
              .map((r) => `<code>${esc(repoName(s, r))}</code>`)
              .join(", ") || "—",
          ],
          [
            "Owns services",
            t.ownsServices
              .map((v) => `<code>${esc(svcName(s, v))}</code>`)
              .join(", ") || "—",
          ],
        ]),
      ),
  ...
  const userRows = s.users
    .map((u) =>
      W.tr([
        ...
        esc(u.role),
        u.permissions.map((p) => W.pill("neutral", p)).join(" "),
      ]),
    )
    .join("");
  return W.page(ctx.app.config, {
