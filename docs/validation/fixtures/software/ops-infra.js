...
function promoteDeployment(ctx) {
  ...
  if (!dep)
    throw new R.AppError(
      ...
      `Deployment "${b.deploymentId}" does not exist.`,
    );
  ...
  if (ctx.failure === "data_conflict") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Deployment ${dep.id} was already promoted by another release manager (simulated).`,
      { deploymentId: dep.id, simulated: true },
    );
  }
  ...
  if (dep.env !== "staging" || dep.status !== "succeeded") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Deployment ${dep.id} is env "${dep.env}" / status "${dep.status}" — only successful staging deployments can be promoted.`,
      { deploymentId: dep.id, env: dep.env, status: dep.status },
    );
  }
  ...
  dep.status = "promoted";
  dep.promotedById = ctx.user.id;
  repo.productionVersion = dep.version;
  const target =
    ctx.state.services.find((s) => s.name === repo.serviceName) || null;
  if (target) target.version = dep.version;
  ctx.emit("deployment.promoted", {
    subject: prodDep.id,
    summary: `${repo.name} ${dep.version} promoted to production by ${ctx.user.name} (from ${dep.id})`,
    data: {
      deploymentId: prodDep.id,
  ...
  if (target) {
    ctx.emit("service.version_bumped", {
      subject: target.id,
      summary: `Service ${target.name} now serving version ${dep.version}`,
  ...
  const n = team
    ? ctx.notify(
        team.memberIds[0],
        "deploy",
        `${repo.name} ${dep.version} is live in production`,
        `Promoted by ${ctx.user.name} from staging deployment ${dep.id}.`,
      )
    : null;
  return {
    ...
    warning:
      n && n.status === "failed"
        ? "Promotion completed, but the team notification could not be delivered."
        : null,
...
function rollbackDeployment(ctx) {
  ...
  if (ctx.failure === "data_conflict") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Rollback rejected (simulated): another operator is mid-rollback for ${repoName(ctx, dep)}.`,
      { deploymentId: dep.id, simulated: true },
    );
  }
  if (dep.env !== "production") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Deployment ${dep.id} is env "${dep.env}" — only production deployments can be rolled back.`,
      { deploymentId: dep.id, env: dep.env },
    );
  }
  const repo = find(ctx.state.repos, dep.repoId);
  const prior = ctx.state.deployments
    .filter(
      (d) =>
        d.repoId === repo.id &&
        d.env === "production" &&
  ...
  if (!prior) {
    throw new R.AppError(
      409,
      "data_conflict",
      `No prior production deployment exists for ${repo.name} — nothing to roll back to.`,
      { deploymentId: dep.id },
    );
  }
  const rb = {
  ...
  ctx.emit("deployment.rolled_back", {
    subject: rb.id,
    summary: `${repo.name} rolled back from ${dep.version} to ${prior.version} by ${ctx.user.name}`,
    data: {
      deploymentId: rb.id,
      rollbackOf: dep.id,
      repoId: repo.id,
  ...
  const n = ctx.notify(
    "u-4",
    "deploy",
    `${repo.name} rolled back to ${prior.version}`,
  ...
  return {
    ...
    warning:
      n && n.status === "failed"
        ? "Rollback completed, but the on-call notification could not be delivered."
        : null,
    redirect: "/deployments",
    deployment: rb,
  };
...
function openIncident(ctx) {
  ...
  if (!service)
    throw new R.AppError(
      404,
      "service_not_found",
      `Service "${b.serviceId}" does not exist.`,
    );
  const sev = ["sev1", "sev2", "sev3"].includes(b.severity)
    ? b.severity
    : "sev3";
  const inc = {
    id: R.nextId(ctx.state, "incSeq", "inc-"),
    severity: sev,
    title: String(b.title || "Untitled incident"),
    ...
    timeline: [
      {
        at: new Date().toISOString(),
        byId: ctx.user.id,
        text: "Opened from the incident console.",
      },
    ],
  };
  ...
  ctx.emit("incident.opened", {
    subject: inc.id,
    summary: `${sev} incident "${inc.title}" opened on ${service.name} by ${ctx.user.name}`,
  ...
  return {
    ...
    warning:
      n && n.status === "failed"
        ? "Incident opened, but the on-call page could not be delivered."
        : null,
    redirect: "/incidents",
    incident: inc,
  };
...
function resolveIncident(ctx) {
  ...
  if (!inc)
    throw new R.AppError(
      404,
      "incident_not_found",
      `Incident "${b.incidentId}" does not exist.`,
    );
  if (ctx.failure === "data_conflict") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Incident ${inc.id} was already resolved by another responder (simulated).`,
  ...
  if (inc.status === "resolved") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Incident ${inc.id} is already resolved.`,
      { incidentId: inc.id },
    );
  }
  inc.status = "resolved";
  ...
  inc.timeline.push({
    at: inc.resolvedAt,
    byId: ctx.user.id,
    text: String(b.resolution || "Resolved."),
  });
  const service = find(ctx.state.services, inc.serviceId);
  if (
    service &&
    service.status !== "healthy" &&
    !ctx.state.incidents.some(
  ...
  ctx.emit("incident.resolved", {
    subject: inc.id,
    summary: `Incident ${inc.id} "${inc.title}" resolved by ${ctx.user.name}`,
  ...
  const n = ctx.notify(
    ...
    `${ctx.user.name}: ${b.resolution || "Resolved."}`,
  );
  return {
    message: `Incident ${inc.id} resolved; ${service ? service.name + " is " + service.status : "service updated"}.`,
    warning:
      n && n.status === "failed"
        ? "Incident resolved, but the reporter notification could not be delivered."
        : null,
    redirect: "/incidents",
    incident: inc,
  };
...
function updateDoc(ctx) {
  ...
  if (!doc)
    throw new R.AppError(
      404,
      "doc_not_found",
      `Document "${b.docId}" does not exist.`,
    );
  if (ctx.failure === "data_conflict") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Document ${doc.id} was edited by someone else while you were saving (simulated optimistic-concurrency mismatch).`,
      { docId: doc.id, currentVersion: doc.version, simulated: true },
    );
  }
  ...
  ctx.emit("doc.updated", {
    subject: doc.id,
    summary: `Doc "${doc.title}" updated to v${doc.version} by ${ctx.user.name}`,
    data: { docId: doc.id, version: doc.version },
  });
  return {
    message: `Document "${doc.title}" saved as v${doc.version}.`,
