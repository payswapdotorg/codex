...
function configPatch(b, op) {
  ...
  const reserved = Object.keys(raw).filter(
    (k) => RESERVED_CONFIG_KEYS.includes(k) || String(k).startsWith("__"),
  );
  if (reserved.length) {
    throw new R.AppError(
      400,
...
function entitlementFor(state, orgId, packageId) {
  ...
  const byNewest = (a, b) => {
    const ga = String(a.grantedAt || ""),
      gb = String(b.grantedAt || "");
  ...
  const records = state.entitlements.filter(
    (e) => e.orgId === orgId && e.packageId === packageId,
  );
...
function installPackage(ctx) {
  const b = ctx.body;
  const pkg =
    find(ctx.state.packages, b.packageId) ||
    ctx.state.packages.find((p) => p.slug === b.slug);
  if (!pkg)
    throw new R.AppError(
      404,
      "package_not_found",
      `Package "${b.packageId || b.slug}" does not exist.`,
    );
  ...
  if (!orgId)
    throw new R.AppError(
      403,
      "permission_denied",
      "Your user is not an admin/member of an organization; installs belong to orgs.",
  ...
  if (ctx.failure === "missing_asset") {
    throw new R.AppError(
      ...
      {
        packageId: pkg.id,
        version: b.expectedVersion || latest.version,
        simulated: true,
      },
    );
  }
  const ent = checkEntitlement(ctx, orgId, pkg, "installation");
  const existing = ctx.state.installs.find(
    (i) => i.orgId === orgId && i.packageId === pkg.id && i.status === "active",
  );
  ...
  if (b.expectedVersion && String(b.expectedVersion) !== latest.version) {
    throw new R.AppError(
      ...
      `Version mismatch: you selected ${b.expectedVersion} but the latest published version is ${latest.version}.`,
      {
        ...
        yourVersion: String(b.expectedVersion),
      },
    );
  }
  let entitlement = ent;
  if (!entitlement) {
    ...
    entitlement = {
      id: R.nextId(ctx.state, "entSeq", "ent-"),
      orgId,
      packageId: pkg.id,
      type: "trial",
      seats: null,
      status: "active",
      validUntil: plusDays(today(ctx), 30),
      grantedAt: today(ctx),
      grantedById: null,
    };
    ...
    ctx.emit("entitlement.granted", {
      ...
      summary: `Trial entitlement for "${pkg.name}" auto-granted to ${orgName(ctx.state, orgId)} (until ${entitlement.validUntil})`,
      data: { entitlementId: entitlement.id, orgId, packageId: pkg.id },
    });
  }
  const install = {
    id: R.nextId(ctx.state, "insSeq", "ins-"),
    orgId,
    packageId: pkg.id,
  ...
  ctx.emit("package.installed", {
    subject: install.id,
    summary: `${orgName(ctx.state, orgId)} installed "${pkg.name}" v${latest.version} (${install.id})`,
    data: {
  ...
  const n = ctx.notify(
    pkg.publisherId,
    "marketplace",
    `${orgName(ctx.state, orgId)} installed ${pkg.name}`,
  ...
  return {
    ...
    warning:
      n && n.status === "failed"
        ? "Install recorded, but the publisher notification could not be delivered."
        : null,
    redirect: "/installs",
    install,
    entitlement,
...
function configureInstall(ctx) {
  ...
  if (!inst)
    throw new R.AppError(
      404,
      "install_not_found",
      `Install "${b.installId}" does not exist.`,
    );
  ...
  if (!ent)
    throw new R.AppError(
      409,
      "stale_entitlement",
      `Install ${inst.id} has no active entitlement — configuration is locked.`,
      { installId: inst.id },
    );
  ...
  ctx.emit("install.configured", {
    subject: inst.id,
    summary: `Install ${inst.id} (${pkg.name} v${inst.version}) reconfigured for ${orgName(ctx.state, inst.orgId)}`,
    data: { installId: inst.id, packageId: pkg.id, keys: Object.keys(patch) },
  });
  return {
    message: `Install ${inst.id} configured (${Object.keys(patch).join(", ") || "no keys"}).`,
    redirect: "/installs",
    install: inst,
  };
}
...
function upgradeInstall(ctx) {
  ...
  if (!ent)
    throw new R.AppError(
      409,
      "stale_entitlement",
      `Install ${inst.id} has no active entitlement — upgrades are locked.`,
      { installId: inst.id },
    );
  if (ctx.failure === "data_conflict") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Install ${inst.id} was upgraded by another admin while you were upgrading (simulated).`,
      { installId: inst.id, simulated: true },
    );
  }
  if (
    !Number.isNaN(parseInt(b.expectedCurrentVersion, 10)) &&
    String(b.expectedCurrentVersion) !== inst.version
  ) {
    throw new R.AppError(
      409,
      "data_conflict",
  ...
  if (!rel)
    throw new R.AppError(
      404,
      "version_not_found",
      `Version "${targetVersion}" of "${pkg.name}" does not exist.`,
      { installId: inst.id, version: targetVersion },
    );
  if (targetVersion === inst.version) {
    throw new R.AppError(
      409,
      "duplicate_event",
      `Install ${inst.id} is already at v${targetVersion}.`,
      { installId: inst.id },
    );
  }
  ...
  ctx.emit("install.upgraded", {
    subject: inst.id,
    summary: `${orgName(ctx.state, inst.orgId)} upgraded "${pkg.name}" ${from} → ${targetVersion}`,
    data: {
      installId: inst.id,
      packageId: pkg.id,
      fromVersion: from,
  ...
  const n = ctx.notify(
    org ? org.adminId : null,
    "marketplace",
    `${pkg.name} upgraded to ${targetVersion}`,
  ...
  return {
    ...
    warning:
      n && n.status === "failed"
        ? "Upgrade recorded, but the org-admin notification could not be delivered."
        : null,
    redirect: "/installs",
    install: inst,
  };
...
function rollbackInstall(ctx) {
  ...
  if (!ent)
    throw new R.AppError(
      409,
      "stale_entitlement",
      `Install ${inst.id} has no active entitlement — rollback is locked.`,
      { installId: inst.id },
    );
  if (ctx.failure === "data_conflict") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Install ${inst.id} was rolled back by another admin concurrently (simulated).`,
      { installId: inst.id, simulated: true },
    );
  }
  ...
  const lastUpgrade = [...inst.history]
    .reverse()
    .find((h) => h.action === "upgraded");
  const prevVersion = lastUpgrade ? lastUpgrade.fromVersion : null;
  if (!prevVersion) {
    throw new R.AppError(
      409,
      "data_conflict",
      `Install ${inst.id} has no prior version to roll back to (installed at v${inst.version}, never upgraded).`,
      { installId: inst.id },
    );
  }
  const from = inst.version;
  ...
  ctx.emit("install.rolled_back", {
    subject: inst.id,
    summary: `${orgName(ctx.state, inst.orgId)} rolled back "${pkg.name}" ${from} → ${prevVersion}`,
    data: {
      installId: inst.id,
      packageId: pkg.id,
      fromVersion: from,
  ...
  const n = ctx.notify(
    org ? org.adminId : null,
    "marketplace",
    `${pkg.name} rolled back to ${prevVersion}`,
  ...
  return {
    ...
    warning:
      n && n.status === "failed"
        ? "Rollback recorded, but the org-admin notification could not be delivered."
        : null,
    redirect: "/installs",
    install: inst,
  };
...
function revokeEntitlement(ctx) {
  ...
  if (!ent)
    throw new R.AppError(
      ...
      `Entitlement "${b.entitlementId}" does not exist.`,
    );
  if (ctx.failure === "data_conflict") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Entitlement ${ent.id} was already revoked (simulated).`,
      { entitlementId: ent.id, simulated: true },
    );
  }
  if (ent.status === "revoked") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Entitlement ${ent.id} is already revoked.`,
      { entitlementId: ent.id },
    );
  }
  ent.status = "revoked";
  ...
  ctx.emit("entitlement.revoked", {
    subject: ent.id,
    summary: `Entitlement ${ent.id} for "${pkg ? pkg.name : ent.packageId}" revoked by ${ctx.user.name}: ${ent.revokeReason}`,
  ...
  const n = ctx.notify(
    org ? org.adminId : null,
    "marketplace",
    `Your entitlement for ${pkg ? pkg.name : ent.packageId} was revoked`,
  ...
  return {
    ...
    warning:
      n && n.status === "failed"
        ? "Revocation recorded, but the org-admin notification could not be delivered."
        : null,
...
function grantEntitlement(ctx) {
  ...
  if (!org)
    throw new R.AppError(
      404,
      "org_not_found",
      `Organization "${b.orgId}" does not exist.`,
    );
  const pkg =
    find(ctx.state.packages, b.packageId) ||
    ctx.state.packages.find((p) => p.slug === b.slug);
  ...
  const type = ["subscription", "per-seat", "trial"].includes(b.type)
    ? b.type
    : "subscription";
  const ent = {
    id: R.nextId(ctx.state, "entSeq", "ent-"),
    ...
    status: "active",
    validUntil: String(b.validUntil || plusDays(today(ctx), 365)),
    grantedAt: today(ctx),
  ...
  ctx.emit("entitlement.granted", {
    subject: ent.id,
    summary: `Entitlement ${ent.id} (${type}) for "${pkg.name}" granted to ${org.name} by ${ctx.user.name} (until ${ent.validUntil})`,
    data: {
      entitlementId: ent.id,
      orgId: org.id,
      packageId: pkg.id,
  ...
  const n = ctx.notify(
    org.adminId,
    "marketplace",
    `Your org now has a ${type} license for ${pkg.name}`,
  ...
  return {
    ...
    warning:
      n && n.status === "failed"
        ? "Entitlement granted, but the org-admin notification could not be delivered."
        : null,
    redirect: "/entitlements",
    entitlement: ent,
  };
