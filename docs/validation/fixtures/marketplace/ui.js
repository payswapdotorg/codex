...
function can(ctx, perm) {
  return ctx.user && ctx.user.permissions.includes(perm);
}
function orgOf(ctx) {
...
function dashboard(ctx) {
  ...
  const installsTable = myInstalls.length
    ? table(
        ["Install", "Package", "Version", "Status", "Entitlement", "Config"],
        myInstalls,
        (i) =>
          W.tr([
            `<code>${esc(i.id)}</code>`,
            pkgLink(s, i.packageId),
            `<code>${esc(i.version)}</code>`,
            W.statusPill(i.status),
            entBadge(ctx, i.entitlementId),
            `<code>${esc(JSON.stringify(i.config))}</code>`,
          ]),
      )
  ...
  const entsTable = myEnts.length
    ? table(
        ["Entitlement", "Package", "Type", "Seats", "Status", "Valid until"],
        myEnts,
        (e) =>
          W.tr([
            `<code>${esc(e.id)}</code>`,
            pkgLink(s, e.packageId),
            esc(e.type),
            e.seats || "—",
            W.statusPill(entStatus(ctx, e)),
            esc(e.validUntil),
          ]),
      )
    : "";
...
${W.kpi(
  "Published versions",
  s.packages.reduce((a, p) => a + p.versions.length, 0),
  "immutable",
)}
...
function catalog(ctx) {
  ...
  const results = s.packages.filter(
    (p) =>
      (!cat || p.category === cat) &&
      (!q ||
  ...
  const rows = results
    .map((p) =>
      W.tr([
        `<a href="/listings/${esc(p.slug)}"><b>${esc(p.name)}</b></a>`,
        `<code>${esc(p.slug)}</code>`,
        esc(p.category),
        esc(p.summary),
        esc(p.licenseModel),
        `<code>${esc(C.latestVersion(p).version)}</code>`,
        `${p.stats.installs} · ★${p.stats.stars} (${p.stats.reviews})`,
        canInstall
          ? installForm(ctx, p)
          : `<a class="btn sec mini" href="/listings/${esc(p.slug)}">View</a>`,
      ]),
    )
    .join("");
  return W.page(ctx.app.config, {
...
${W.card(
  "Search the catalog",
  `<form method="get" action="/catalog" class="frm" style="grid-template-columns:2fr 1fr auto;max-width:720px">
<input name="q" value="${esc(q)}" placeholder="Search packages (e.g. invoice, triage)">
<select name="category"><option value="">All categories</option>${s.packages
    .map((p) => p.category)
...
function installForm(ctx, p) {
  ...
  return W.frm(
    "/ui/installs/install",
    [W.hidden("packageId", p.id), W.hidden("expectedVersion", latest.version)],
    {
      submit: "Install " + latest.version,
      note:
        p.licenseModel === "free"
          ? "Free package."
          : "First install auto-grants a 30-day trial entitlement.",
    },
  );
}
...
function listing(ctx) {
  ...
  if (!pkg)
    throw new R.AppError(
      404,
      "listing_not_found",
      `Listing "${ctx.params.slug}" does not exist.`,
    );
  ...
  const overview = W.dl([
    ["ID", `<code>${esc(pkg.id)}</code>`],
    ["Publisher", esc(userName(s, pkg.publisherId))],
    ["Category", esc(pkg.category)],
    ["License model", esc(pkg.licenseModel)],
    ["Latest version", `<code>${esc(latest.version)}</code>`],
    ["Latest digest", `<code>${esc(latest.digest)}</code>`],
    [
      "Stats",
      `${pkg.stats.installs} installs · ★${pkg.stats.stars} · ${pkg.stats.reviews} reviews`,
    ],
    ["Tags", pkg.tags.map((t) => W.pill("info", t)).join(" ") || "—"],
    ["Description", esc(pkg.description)],
  ]);
  const versionsTable = table(
    ["Version", "Released", "Changelog", "Digest", "Commit", "Built by"],
    pkg.versions.slice().reverse(),
    (v) =>
      W.tr([
        `<code>${esc(v.version)}</code>`,
        esc(v.releasedAt),
        esc(v.changelog),
        `<code>${esc(v.digest)}</code>`,
        `<code>${esc(v.provenance.commit)}</code>`,
        esc(v.provenance.builtBy),
      ]),
  );
  const provenanceCard = W.card(
    "Provenance & attribution (latest)",
    W.dl([
      [
        "Source repository",
        `<code>${esc(latest.provenance.sourceRepo)}</code>`,
      ],
      ["Commit", `<code>${esc(latest.provenance.commit)}</code>`],
      ["Built by", esc(latest.provenance.builtBy)],
      ["Signed by", esc(latest.provenance.signedBy)],
      ["Manifest digest", `<code>${esc(latest.digest)}</code>`],
    ]) +
      (canVerify
        ? W.frm(
            "/ui/provenance/verify",
            [
              W.hidden("packageId", pkg.id),
              W.hidden("version", latest.version),
            ],
            {
              submit: "Verify provenance",
              sec: true,
              note: "Recomputes the manifest digest and compares it to the published digest.",
            },
          )
        : ""),
  );
  const installFormHtml = canInstall
    ? W.card(
    ...
    : "";
  const publishVersionForm = canPublishVersion
    ? W.card(
        "Publish a new version (publisher)",
        W.frm(
          "/ui/versions/publish",
          [
            W.hidden("packageId", pkg.id),
            W.fld(
              "Version (semver)",
              W.txt("version", nextPatch(latest.version)),
            ),
            W.fld("Changelog", W.txt("changelog", "")),
          ],
          {
            submit: "Publish version",
            note: "Published versions are immutable; duplicate versions are rejected.",
          },
        ),
      )
    : "";
  const reviewForm = canReview
    ? W.card(
        ...
        W.frm(
          ...
          {
            submit: "Post review",
            note: "One review per organization per package.",
          },
        ),
      )
    : "";
  const reviewsTable = reviews.length
    ? table(["Review", "Org", "By", "Rating", "Text", "At"], reviews, (r) =>
        W.tr([
          `<code>${esc(r.id)}</code>`,
          esc(C.orgName(s, r.orgId)),
          esc(userName(s, r.byId)),
          `${r.rating}/5`,
          esc(r.text),
          `<span class="mono">${esc(r.at)}</span>`,
        ]),
      )
    ...
    ? W.card(
        ...
        W.frm(
          ...
          [
            ...
            W.fld(
              "Source repository",
              W.txt("sourceRepo", "https://git.example.internal/you/package"),
            ),
          ],
          {
            submit: "Publish listing v1.0.0",
            note: "A manifest + sha256 digest + provenance record are created with the first version.",
          },
        ),
      )
    : '<p class="muted">Sign in as a publisher to list packages.</p>';
  return W.page(ctx.app.config, {
...
function installs(ctx) {
  ...
  const cards = mine
    .map((i) => {
      ...
      const latest = C.latestVersion(pkg);
      return W.card(
        `Install <code>${esc(i.id)}</code> — ${esc(pkg.name)} <code>${esc(i.version)}</code> ${W.statusPill(i.status)}`,
        `
${W.dl([
  ["Organization", esc(C.orgName(s, i.orgId))],
  ["Entitlement", entBadge(ctx, i.entitlementId)],
  [
    "Latest published",
    `<code>${esc(latest.version)}</code>${i.version !== latest.version ? " — upgrade available" : " — up to date"}`,
  ],
  ["Installed by", esc(userName(s, i.installedById))],
  ["Installed at", `<span class="mono">${esc(i.installedAt)}</span>`],
...
${
  canManage
    ? W.frm(
        "/ui/installs/configure",
        [
          W.hidden("installId", i.id),
          W.fld(
            "Config keys to merge (JSON)",
            W.ta("config", '{\n  "key": "value"\n}'),
          ),
        ],
...
${
  canManage && i.version !== latest.version
    ? W.frm(
        "/ui/installs/upgrade",
        [
          W.hidden("installId", i.id),
          ...
          W.fld(
            ...
            W.sel(
              "targetVersion",
              pkg.versions
                .filter((v) => v.version !== i.version)
                .map((v) => [v.version, v.version + " — " + v.changelog])
                .reverse(),
            ),
          ),
        ],
...
function entitlements(ctx) {
  ...
  const rows = s.entitlements
    .map((e) =>
      W.tr([
        `<code>${esc(e.id)}</code>`,
        esc(C.orgName(s, e.orgId)),
        pkgLink(s, e.packageId),
        esc(e.type),
        e.seats || "—",
        W.statusPill(entStatus(ctx, e)),
        esc(e.validUntil),
        canRevoke && e.status === "active"
          ? W.frm(
              "/ui/entitlements/revoke",
              [
                W.hidden("entitlementId", e.id),
                W.fld("Reason", W.txt("reason", "Commercial decision.")),
              ],
              { submit: "Revoke" },
            )
          : "—",
    ...
    .join("");
  const grantForm = canGrant
    ? W.card(
        "Grant an entitlement (admin)",
        W.frm(
          "/ui/entitlements/grant",
          [
            W.fld(
              "Organization",
              W.sel(
                "orgId",
                s.orgs.map((o) => [o.id, o.name]),
              ),
            ),
            W.fld(
              "Package",
              W.sel(
                "packageId",
                s.packages.map((p) => [p.id, `${p.name} (${p.slug})`]),
              ),
            ),
            W.fld(
              "Type",
            ...
            W.fld(
              "Seats (per-seat only)",
              W.txt("seats", "10", { type: "number", min: 1 }),
            ),
            W.fld(
              "Valid until",
              W.txt("validUntil", "2027-09-12", { type: "date" }),
            ),
          ],
          { submit: "Grant" },
        ),
