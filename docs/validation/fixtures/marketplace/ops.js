...
function publishListing(ctx) {
  ...
  if (!b.name || !slug)
    throw new R.AppError(
      400,
      "validation_error",
      "Listing name and slug are required.",
    );
  if (ctx.state.packages.find((p) => p.slug === slug)) {
    throw new R.AppError(
      409,
      "duplicate_event",
      `A listing with slug "${slug}" already exists in the catalog.`,
      { slug },
    );
  }
  const manifest = {
  ...
  const pkg = {
    id: R.nextId(ctx.state, "pkSeq", "pk-"),
    slug,
    name: String(b.name),
    publisherId: ctx.user.id,
    category: String(b.category || "general"),
    licenseModel: ["free", "subscription", "per-seat"].includes(b.licenseModel)
      ? b.licenseModel
      : "subscription",
    ...
    versions: [
      {
        ...
        provenance: {
          sourceRepo: String(
            b.sourceRepo ||
              "https://git.example.internal/" + ctx.user.username + "/" + slug,
          ),
          commit: Math.random().toString(16).slice(2, 9),
          builtBy: ctx.user.username,
          signedBy: ctx.user.username,
        },
      },
    ],
  ...
  ctx.emit("listing.published", {
    subject: pkg.id,
    summary: `Listing "${pkg.name}" (${slug}) v1.0.0 published by ${ctx.user.name}`,
    data: {
      packageId: pkg.id,
      slug,
      version: "1.0.0",
      digest: pkg.versions[0].digest,
    },
  });
  const n = ctx.notify(
    "u-2",
    "marketplace",
    `New listing: ${pkg.name}`,
    `${ctx.user.name} published "${pkg.name}" (${slug}).`,
  );
  return {
    message: `Listing ${pkg.id} ("${pkg.name}") published at v1.0.0.`,
    warning:
      n && n.status === "failed"
        ? "Listing published, but the admin notification could not be delivered."
        : null,
...
function publishVersion(ctx) {
  ...
  if (ctx.failure === "data_conflict") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Another publisher session already released a version for "${target.name}" (simulated).`,
      { packageId: target.id, simulated: true },
    );
  }
  ...
  if (target.versions.find((v) => v.version === version)) {
    throw new R.AppError(
      409,
      "duplicate_event",
      `Version ${version} of "${target.name}" already exists — published versions are immutable.`,
      { packageId: target.id, version },
    );
  }
  ...
  const release = {
    ...
    digest: digestOf(manifest),
    provenance: {
      sourceRepo: prev.provenance.sourceRepo,
      commit: Math.random().toString(16).slice(2, 9),
      builtBy: ctx.user.username,
      signedBy: ctx.user.username,
    },
  ...
  ctx.emit("version.published", {
    subject: `${target.id}@${version}`,
    summary: `"${target.name}" ${version} released by ${ctx.user.name}: ${release.changelog}`,
    data: { packageId: target.id, version, digest: release.digest },
  });
  // notify org admins with active installs (upgrade available)
  const installers = ctx.state.installs.filter(
    (i) => i.packageId === target.id && i.status === "active",
  );
  const warnings = [];
  for (const inst of installers) {
    ...
    const n = ctx.notify(
      ...
      "marketplace",
      `Upgrade available: ${target.name} ${version}`,
  ...
  return {
    ...
    warning: warnings.length
      ? `Released, but upgrade notifications failed for: ${warnings.join(", ")}.`
      : null,
    redirect: `/listings/${target.slug}`,
    release,
  };
}
...
function verifyProvenance(ctx) {
  ...
  if (!rel)
    throw new R.AppError(
      404,
      "version_not_found",
      `Version "${version}" of "${pkg.name}" does not exist.`,
      { packageId: pkg.id, version },
    );
  if (ctx.failure === "data_conflict") {
    throw new R.AppError(
      409,
  ...
  ctx.emit("provenance.verified", {
    ...
    data: {
      ...
      match,
      digest: recomputed,
      commit: rel.provenance.commit,
      builtBy: rel.provenance.builtBy,
    },
  });
  if (!match) {
    throw new R.AppError(
      409,
      "data_conflict",
      `Provenance mismatch: recomputed digest ${recomputed} != published ${rel.digest}.`,
      { packageId: pkg.id, version },
    );
  }
  return {
...
function writeReview(ctx) {
  ...
  if (!orgId)
    throw new R.AppError(
      403,
      "permission_denied",
      "Reviews must come from an organization member; your user is not in any org.",
      { user: ctx.user.username },
    );
  if (
    ctx.state.reviews.find((r) => r.packageId === pkg.id && r.orgId === orgId)
  ) {
    throw new R.AppError(
      409,
      "duplicate_event",
      `Your organization already reviewed "${pkg.name}" — one review per org per package.`,
      { packageId: pkg.id, orgId },
    );
  }
  const rating = Math.max(1, Math.min(5, parseInt(b.rating, 10) || 5));
  const review = {
    id: R.nextId(ctx.state, "revSeq", "rv-"),
    packageId: pkg.id,
    orgId,
    byId: ctx.user.id,
    rating,
  ...
  pkg.stats.reviews += 1;
  const total = ctx.state.reviews
    .filter((r) => r.packageId === pkg.id)
    .reduce((a, r) => a + r.rating, 0);
  pkg.stats.stars = Math.round((total / pkg.stats.reviews) * 10) / 10;
  ctx.emit("review.posted", {
    subject: review.id,
    summary: `${orgName(ctx.state, orgId)} rated "${pkg.name}" ${rating}/5`,
    data: { reviewId: review.id, packageId: pkg.id, rating },
  });
  const n = ctx.notify(
    pkg.publisherId,
    "marketplace",
    `New ${rating}★ review on ${pkg.name}`,
    `${orgName(ctx.state, orgId)}: "${review.text.slice(0, 90)}"`,
  );
  return {
    message: `Review ${review.id} posted (${rating}/5).`,
    warning:
      n && n.status === "failed"
        ? "Review posted, but the publisher notification could not be delivered."
        : null,
    redirect: `/listings/${pkg.slug}`,
    review,
  };
}
...
function searchCatalog(ctx) {
  ...
  if (q) {
    results = results
      .map((p) => {
        const hay =
          `${p.name} ${p.slug} ${p.summary} ${p.description} ${p.tags.join(" ")}`.toLowerCase();
        let score = 0;
