...
function submitDriverApplication(ctx) {
  ...
  if (!region)
    throw new R.AppError(
      404,
      "region_not_found",
      `Region "${b.regionId}" does not exist.`,
    );
  ...
  if (!b.name || !plate)
    throw new R.AppError(
      400,
      "validation_error",
      "Name and vehicle plate are required.",
    );
  if (
    ctx.state.driverApplications.find(
      (a) =>
        a.vehicle.plate === plate &&
        ["submitted", "screening", "approved"].includes(a.status),
    )
  ) {
    throw new R.AppError(
      409,
  ...
  const app = {
    id: R.nextId(ctx.state, "appSeq", "da-"),
    name: String(b.name),
    phoneMasked: "***-***-" + String(b.phone || "0000").slice(-4),
    email: String(b.email || ""),
    regionId: region.id,
    vehicle: {
      make: String(b.make || "—"),
      model: String(b.model || "—"),
      year: parseInt(b.year, 10) || 2021,
      plate,
    },
    docs: [
    ...
    reviewedById: null,
    decisionNotes: "",
    preIssuedPermitValidUntil: plusDays(today(ctx), 365),
  };
  ctx.state.driverApplications.push(app);
  ctx.emit("driver.application_submitted", {
    subject: app.id,
    summary: `Driver application ${app.id} (${app.name}, ${region.code}) submitted via public intake`,
    data: { applicationId: app.id, regionId: region.id, plate },
  });
  const n = ctx.notify(
    "u-2",
    "onboarding",
    `New driver application: ${app.name}`,
  ...
  return {
    ...
    warning:
      n && n.status === "failed"
        ? "Application recorded, but the ops notification could not be delivered."
        : null,
    redirect: "/intake",
    application: app,
  };
...
function screenDriverApplication(ctx) {
  ...
  if (!app)
    throw new R.AppError(
      ...
      `Driver application "${b.applicationId}" does not exist.`,
    );
  if (ctx.failure === "data_conflict") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Application ${app.id} was already decided by another ops reviewer (simulated).`,
  ...
  if (app.status !== "submitted" && app.status !== "screening") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Application ${app.id} is already "${app.status}" — it cannot be screened again.`,
  ...
  if (decision === "approve") {
    ...
    if (ctx.failure === "missing_asset" || missingDoc) {
      throw new R.AppError(
        ...
        `Required document "${missingDoc ? missingDoc.kind : "insurance"}" is not on file for application ${app.id}${missingDoc ? "" : " (simulated dangling reference)"} — approval blocked.`,
        {
          ...
          document: missingDoc ? missingDoc.kind : "insurance",
          simulated: !missingDoc || undefined,
        },
      );
    }
    app.status = "approved";
    ...
    ctx.emit("driver.application_approved", {
      subject: app.id,
      summary: `Driver application ${app.id} (${app.name}) approved by ${ctx.user.name}`,
      data: { applicationId: app.id, regionId: app.regionId },
    });
    const n = ctx.notify(
      "u-1",
      "onboarding",
      `Application ${app.id} approved`,
    ...
    return {
      ...
      warning:
        n && n.status === "failed"
          ? "Approval recorded, but the director notification could not be delivered."
          : null,
      redirect: "/onboarding",
      application: app,
    };
  ...
  ctx.emit("driver.application_rejected", {
    subject: app.id,
    summary: `Driver application ${app.id} (${app.name}) rejected by ${ctx.user.name}`,
    data: { applicationId: app.id, notes: app.decisionNotes },
  });
  return {
    message: `Application ${app.id} rejected.`,
    redirect: "/onboarding",
    application: app,
  };
}
...
function activateDriver(ctx) {
  ...
  if (app.status !== "approved") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Application ${app.id} is "${app.status}" — only approved applications can be activated.`,
      { applicationId: app.id, status: app.status },
    );
  }
  if (isPast(ctx, app.preIssuedPermitValidUntil)) {
    throw new R.AppError(
      ...
      {
        applicationId: app.id,
        permitValidUntil: app.preIssuedPermitValidUntil,
      },
    );
  }
  const driver = {
    id: R.nextId(ctx.state, "driverSeq", "dr-"),
    applicationId: app.id,
    name: app.name,
  ...
  ctx.emit("driver.activated", {
    subject: driver.id,
    summary: `Driver ${driver.name} activated in ${region.name} by ${ctx.user.name} (permit until ${driver.permitValidUntil})`,
  ...
  const n = ctx.notify(
    "u-3",
    "onboarding",
    `Driver ${driver.name} activated`,
    `${ctx.user.name} activated ${driver.name} in ${region.name}.`,
  );
  return {
    message: `Driver ${driver.name} activated in ${region.name} (driver id ${driver.id}).`,
    warning:
      broadcast.status === "failed"
        ? "Driver activated, but the welcome broadcast failed to deliver (notification_failure)."
        : null,
    redirect: "/onboarding",
    driver,
    broadcast,
...
function respondTicket(ctx) {
  ...
  if (!t)
    throw new R.AppError(
      404,
      "ticket_not_found",
      `Ticket "${b.ticketId}" does not exist.`,
    );
  if (t.status === "resolved")
    throw new R.AppError(
      409,
      "data_conflict",
      `Ticket ${t.id} is already resolved.`,
      { ticketId: t.id },
    );
  t.messages.push({
  ...
  ctx.emit("support.message_added", {
    subject: t.id,
    summary: `${ctx.user.name} replied on ticket ${t.id} (${t.subject})`,
    data: { ticketId: t.id },
  });
  return {
    message: `Reply added to ticket ${t.id}.`,
    redirect: "/support",
    ticket: t,
  };
}
...
function escalateTicket(ctx) {
  ...
  if (ctx.failure === "data_conflict") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Ticket ${t.id} was already escalated or resolved by another agent (simulated).`,
      { ticketId: t.id, simulated: true },
    );
  }
  if (t.status !== "open") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Ticket ${t.id} is "${t.status}" — only open tickets can be escalated.`,
  ...
  t.messages.push({
    ...
    role: "agent",
    at: new Date().toISOString(),
    text: `Escalated: ${b.reason || "needs ops attention"}`,
  });
  ctx.emit("support.ticket_escalated", {
    subject: t.id,
    summary: `Ticket ${t.id} escalated by ${ctx.user.name} — ${b.reason || "needs ops attention"}`,
    data: { ticketId: t.id, priority: t.priority },
  });
  const n = ctx.notify(
    "u-2",
    "support",
    `Ticket ${t.id} escalated: ${t.subject}`,
  ...
  return {
    ...
    warning:
      n && n.status === "failed"
        ? "Escalation recorded, but the ops notification could not be delivered."
        : null,
    redirect: "/support",
    ticket: t,
  };
...
function resolveTicket(ctx) {
  ...
  if (ctx.failure === "data_conflict") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Ticket ${t.id} was resolved by another agent (simulated).`,
      { ticketId: t.id, simulated: true },
    );
  }
  if (t.status === "resolved")
  ...
  t.messages.push({
    ...
    role: "agent",
    at: new Date().toISOString(),
    text: `Resolved: ${b.resolution || "handled"}`,
  });
  ctx.emit("support.ticket_resolved", {
    subject: t.id,
    summary: `Ticket ${t.id} (${t.subject}) resolved by ${ctx.user.name}`,
    data: { ticketId: t.id },
  });
  return {
