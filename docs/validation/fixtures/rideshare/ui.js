...
function dashboard(ctx) {
  const s = ctx.state;
  const pendingApps = s.driverApplications.filter((a) =>
    ["submitted", "screening"].includes(a.status),
  );
  const openTickets = s.supportTickets.filter((t) => t.status !== "resolved");
  const regionsTable = table(
    ["Region", "Code", "Surge", "Active drivers", "Demand", "Status"],
    s.regions,
    (r) =>
      W.tr([
        `<b>${esc(r.name)}</b>`,
        `<code>${esc(r.code)}</code>`,
        `${r.surgeLevel.toFixed(1)}×`,
        r.activeDrivers,
        W.progress(r.demandIndex) +
          ` <span class="muted">${r.demandIndex}</span>`,
...
${W.card(
  "Recent activity",
  table(["Time", "Type", "Summary"], recent, (e) =>
    W.tr([
...
function onboarding(ctx) {
    ...
    .map((a) =>
      W.tr([
        ...
        canActivate && a.status === "approved"
          ? W.frm("/ui/drivers/activate", [W.hidden("applicationId", a.id)], {
              submit: "Activate driver",
            })
          : "—",
      ]),
    )
  ...
  const driversTable = table(
    ...
    (d) =>
      W.tr([
        `<code>${esc(d.id)}</code> ${esc(d.name)}`,
        esc(regionName(s, d.regionId)),
        W.statusPill(d.status),
        d.rating ? d.rating.toFixed(2) : "—",
        d.tripsCompleted,
        money(d.earningsCents),
        esc(d.permitValidUntil),
      ]),
  );
  return W.page(ctx.app.config, {
...
${W.card(
  "Driver application (public intake)",
  W.frm(
    "/ui/intake/apply",
    [
      W.fld("Full name", W.txt("name", "", { required: true })),
      W.fld(
        "Phone (last 4 shown only)",
        W.txt("phone", "", { required: true }),
      ),
      ...
      W.fld(
        ...
        W.sel(
          "regionId",
          s.regions.map((r) => [r.id, `${r.name} (${r.code})`]),
        ),
      ),
      ...
      W.fld(
        "Vehicle: year",
        W.txt("year", "2021", { type: "number", min: 2005, max: 2027 }),
      ),
      W.fld("Vehicle: plate", W.txt("plate", "", { required: true })),
      W.fld(
        "Do you have your license document ready?",
        W.sel("licenseDoc", [
          ["yes", "Yes"],
          ["no", "No — not yet"],
        ]),
      ...
      W.fld(
        "Do you have your insurance document ready?",
        W.sel("insuranceDoc", [
          ["yes", "Yes"],
          ["no", "No — not yet"],
        ]),
    ...
    ],
    {
      ...
      note: "This public form is the natural duplicate_event case: submitting the same plate twice is rejected.",
    },
  ),
)}
...
function support(ctx) {
  ...
  const cards = s.supportTickets
    .map((t) =>
      W.card(
        `Ticket ${esc(t.id)} — ${esc(t.subject)} ${W.statusPill(t.status)}`,
        `
${W.dl([
  ["Opened by", esc(t.openedBy)],
  ["Category", esc(t.category)],
  ["Priority", W.statusPill(t.priority)],
  ["Assignee", esc(t.assigneeId ? userName(s, t.assigneeId) : "—")],
])}
...
${
  canEscalate && t.status === "open"
    ? W.frm(
        "/ui/tickets/escalate",
        [
          W.hidden("ticketId", t.id),
          W.fld("Escalation reason", W.txt("reason", "Needs ops attention")),
        ],
...
${
  canResolve && t.status !== "resolved"
    ? W.frm(
        ...
        [
          W.hidden("ticketId", t.id),
          W.fld("Resolution", W.txt("resolution", "Handled.")),
        ],
        { submit: "Resolve", sec: true },
      )
    : ""
}`,
      ),
...
function ops(ctx) {
  ...
  const surgeForm = canSurge
    ? W.card(
        "Adjust surge (ops decision workflow)",
        W.frm(
          "/ui/regions/surge",
          [
            W.fld(
              ...
              W.sel(
                ...
                s.regions.map((r) => [
                  r.id,
                  `${r.name} (${r.surgeLevel.toFixed(1)}×)`,
                ]),
              ),
            ),
            W.fld(
              "New surge level (1.0 – 3.0)",
              W.txt("level", "1.5", {
                type: "number",
                min: 1.0,
                max: 3.0,
          ...
          {
            submit: "Apply surge",
            note: "Broadcasts the change to region drivers (notification_failure makes the broadcast fail).",
          },
        ),
      )
    : "";
  const bTable = table(
    ["Broadcast", "Region", "Channel", "Message", "Status", "Sent by", "At"],
    s.broadcasts.slice().reverse(),
    (b) =>
      W.tr([
        `<code>${esc(b.id)}</code>`,
        esc(regionName(s, b.regionId)),
        esc(b.channel),
        esc(b.message),
        W.statusPill(b.status),
        esc(userName(s, b.sentById)),
        `<span class="mono">${esc(b.at)}</span>`,
      ]),
  );
  return W.page(ctx.app.config, {
    title: "Operations",
...
function map(ctx) {
  ...
  const CELL = 100,
    PAD = 4;
  const rects = s.regions
    .map((r) => {
      const x = r.map.x * CELL + PAD,
        y = r.map.y * CELL + PAD,
        w = r.map.w * CELL - 2 * PAD,
        h = r.map.h * CELL - 2 * PAD;
      const dots = [];
      ...
      for (let i = 0; i < n; i++) {
        ...
        dots.push(
          `<circle cx="${dx}" cy="${dy}" r="4" fill="#0f766e" opacity="0.75"><title>driver</title></circle>`,
        );
      }
...
${W.card(
  "Regions, surge state and driver presence",
  `
...
function payments(ctx) {
  ...
  const rows = s.payments
    .map((p) =>
      W.tr([
        `<code>${esc(p.id)}</code>`,
        driverCell(s, p.driverId),
        esc(p.payPeriod),
        p.tripsCount,
        money(p.grossCents),
        money(p.commissionCents),
        `<b>${money(p.netCents)}</b>`,
        W.statusPill(p.status),
        canApprove && p.status === "pending"
          ? W.frm("/ui/payments/approve", [W.hidden("paymentId", p.id)], {
...
function driverCell(s, id) {
  ...
  return d
    ? `<code>${esc(d.id)}</code> ${esc(d.name)} <span class="muted">permit ${esc(d.permitValidUntil)}</span>`
    : id;
}
...
function incidents(ctx) {
  ...
  const rows = s.incidents
    .map((i) =>
      W.tr([
        `<code>${esc(i.id)}</code>`,
        i.tripId ? `<code>${esc(i.tripId)}</code>` : "—",
        driverCell(s, i.driverId),
        esc(i.category),
        ...
        canResolve && i.status !== "resolved"
          ? W.frm(
              "/ui/incidents/resolve",
              [
                W.hidden("incidentId", i.id),
                W.fld(
                  "Resolution",
                  W.txt("resolution", "Reviewed and closed."),
                ),
              ],
              { submit: "Resolve" },
            )
          ...
          [
            W.fld(
              ...
              W.sel(
                "driverId",
                [["", "— no driver —"]].concat(
                  s.drivers.map((d) => [d.id, `${d.id} — ${d.name}`]),
                ),
              ),
            ),
