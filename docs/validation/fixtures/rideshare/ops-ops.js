...
function adjustSurge(ctx) {
  ...
  if (ctx.failure === "data_conflict") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Another ops manager adjusted ${region.name} surge while you were saving (simulated).`,
      { regionId: region.id, simulated: true },
    );
  }
  ...
  if (Number.isNaN(level) || level < 1.0 || level > 3.0) {
    throw new R.AppError(
      400,
      "validation_error",
      "Surge level must be between 1.0 and 3.0.",
    );
  }
  ...
  ctx.emit("ops.surge_adjusted", {
    subject: region.id,
    summary: `${region.name} surge ${before.toFixed(1)}× → ${level.toFixed(1)}× by ${ctx.user.name}`,
    data: {
      regionId: region.id,
      from: before,
      to: level,
  ...
  return {
    ...
    warning:
      broadcast.status === "failed"
        ? "Surge updated, but the driver broadcast could not be delivered (notification_failure)."
        : null,
    redirect: "/ops",
    region,
    broadcast,
...
function approvePayout(ctx) {
  ...
  if (!pay)
    throw new R.AppError(
      404,
      "payment_not_found",
      `Payment "${b.paymentId}" does not exist.`,
    );
  ...
  if (ctx.failure === "data_conflict") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Payout ${pay.id} was already processed by another analyst (simulated).`,
      { paymentId: pay.id, simulated: true },
    );
  }
  if (pay.status !== "pending") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Payout ${pay.id} is already "${pay.status}".`,
      { paymentId: pay.id, status: pay.status },
    );
  }
  if (
    ctx.failure === "stale_entitlement" ||
    (driver && isPast(ctx, driver.permitValidUntil))
  ) {
    throw new R.AppError(
      409,
      "stale_entitlement",
      `Driver ${driver ? driver.name : pay.driverId}'s regional permit expired on ${driver ? driver.permitValidUntil : "?"} — payouts require a valid permit.`,
      {
        ...
        permitValidUntil: driver && driver.permitValidUntil,
        simulated: ctx.failure === "stale_entitlement" || undefined,
      },
    );
  }
  pay.status = "approved";
  pay.approvedById = ctx.user.id;
  pay.approvedAt = new Date().toISOString();
  ctx.emit("payout.approved", {
    subject: pay.id,
    summary: `Payout ${pay.id} to ${driver ? driver.name : pay.driverId} ($${(pay.netCents / 100).toFixed(2)}) approved by ${ctx.user.name}`,
    data: { paymentId: pay.id, driverId: pay.driverId, netCents: pay.netCents },
  });
  return {
    message: `Payout ${pay.id} approved for payment.`,
    redirect: "/payments",
    payment: pay,
  };
}
...
function reportIncident(ctx) {
  ...
  if (b.driverId && !driver)
    throw new R.AppError(
      404,
      "driver_not_found",
      `Driver "${b.driverId}" does not exist.`,
    );
  const inc = {
    id: R.nextId(ctx.state, "incSeq", "in-"),
    tripId: b.tripId || null,
    driverId: driver ? driver.id : null,
    category: String(b.category || "other"),
    severity: ["low", "medium", "high"].includes(b.severity)
      ? b.severity
      : "low",
  ...
  ctx.emit("incident.reported", {
    subject: inc.id,
    summary: `${inc.severity} incident (${inc.category}) reported${driver ? " on driver " + driver.name : ""}`,
    data: { incidentId: inc.id, driverId: inc.driverId, tripId: inc.tripId },
  });
  const n = ctx.notify(
    "u-1",
    "incident",
    `New ${inc.severity} incident ${inc.id}`,
  ...
  return {
    ...
    warning:
      n && n.status === "failed"
        ? "Incident saved, but the ops-director notification could not be delivered."
        : null,
    redirect: "/incidents",
    incident: inc,
  };
...
function resolveIncident(ctx) {
  ...
  if (ctx.failure === "data_conflict") {
    throw new R.AppError(
      409,
      "data_conflict",
      `Incident ${inc.id} was resolved by another ops manager (simulated).`,
  ...
  ctx.emit("incident.resolved", {
    subject: inc.id,
    summary: `Incident ${inc.id} resolved by ${ctx.user.name}: ${inc.resolution}`,
