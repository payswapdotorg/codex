...
class AppError extends Error {
  ...
  toJSON() {
    return Object.assign(
      { ok: false, error: this.code, message: this.message },
      this.extra,
    );
  }
}
...
function createStore(rootDir, seedFn) {
  ...
  function ensureMeta(s) {
    s.meta = Object.assign(
      { eventSeq: 0, notifSeq: 0, resets: 0, idempotency: {} },
      s.meta || {},
    );
    return s;
  }
  function seed() {
    ...
    if (!Array.isArray(state.users))
      throw new Error("seed must define state.users[]");
...
function demoHints(state) {
  return state.users.map((u) => ({
    username: u.username,
    password: assemble(u.passwordParts),
    name: u.name,
    role: u.role,
  }));
}
function checkLogin(state, username, password) {
  ...
  if (!u || assemble(u.passwordParts) !== String(password)) {
    throw new AppError(
      401,
      "invalid_credentials",
      "Unknown user or wrong password (demo credentials are listed on the login page).",
    );
  }
...
function emit(ctx, type, fields) {
  return pushEvent(
    ctx.state,
    ctx.user ? ctx.user.username : "system",
    type,
    fields,
  );
}
...
function signature(body) {
  ...
  const keys = Object.keys(body)
    .filter((k) => k !== "idempotencyKey" && k !== "idempotency_key")
    .sort();
...
function checkDuplicate(ctx, opKey, naturalKey) {
  ...
  const provided =
    ctx.body && (ctx.body.idempotencyKey || ctx.body.idempotency_key);
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on("data", (c) => {
      ...
      if (size > MAX_BODY_BYTES) {
        reject(
          new AppError(413, "payload_too_large", "Request body exceeds 1 MB."),
        );
        req.destroy();
        return;
      }
...
function createApp(config) {
  ...
  for (const k of required)
    if (config[k] === undefined)
      throw new Error(`createApp: missing config.${k}`);
  ...
  function userFor(req, url) {
    ...
    const token =
      ...
      parseCookies(req)[app.cookieName] ||
      null;
  ...
  function resetWorld() {
    ...
    pushEvent(s, null, "world.reset", {
      subject: "state",
      summary: `Demo world reset to seed state (reset #${s.meta.resets})`,
  ...
  const standardRoutes = [
    {
      ...
      op: () => ({
        ...
        resets: store.state.meta.resets,
      }),
    },
    {
      kind: "api",
    ...
    },
    {
      ...
      op: (ctx) => {
        ...
        pushEvent(store.state, r.user.username, "auth.login", {
          subject: `user:${r.user.id}`,
          summary: `${r.user.username} signed in`,
          data: { role: r.user.role },
        });
        return {
          ok: true,
          token: r.token,
          user: r.user,
          note: 'Send this token as "Authorization: Bearer <token>" or "?token=".',
        };
      },
    },
    {
    ...
    },
    {
      ...
      op: () => ({
        ok: true,
        usage:
          'Append ?failure=<switch> to any request, or send header "X-Failure-Switch: <switch>".',
    ...
    },
    {
      ...
      op: () => {
        ...
        return {
          ok: true,
          message: "Seed state restored.",
          resets: s.meta.resets,
    ...
    },
    {
      ...
      pattern: "/login",
      auth: false,
      handler: (ctx) => web.loginPage(ctx, config, demoHints(store.state)),
    },
    {
      kind: "page",
      method: "GET",
    ...
    },
    {
      ...
      pattern: "/failures",
      auth: false,
      handler: (ctx) => web.failuresPage(ctx, config),
    },
    {
      kind: "form",
      method: "POST",
      ...
      op: (ctx) => {
        ...
        ctx.res.setHeader(
          "Set-Cookie",
          `${app.cookieName}=${r.token}; Path=/; HttpOnly; SameSite=Lax`,
        );
        const next = String(ctx.body.next || "/");
        return {
          redirect: next.startsWith("/") ? next : "/",
          message: `Signed in as ${r.user.name} (${r.user.role}).`,
        };
      },
      back: "/login",
    },
    {
      ...
      op: (ctx) => {
        ctx.res.setHeader(
          "Set-Cookie",
          `${app.cookieName}=; Path=/; HttpOnly; Max-Age=0`,
        );
    ...
    },
    {
      ...
      op: () => {
        ...
        return {
          redirect: "/",
          message: `Demo world reset to seed state (reset #${s.meta.resets}). You were signed out.`,
        };
      },
      back: "/",
    },
    ...
    try {
      ...
      const err =
        e instanceof AppError
          ? e
          : new AppError(500, "internal_error", String((e && e.message) || e));
      if (!(e instanceof AppError))
        console.error("[runtime] internal error:", e);
      ...
      if (err.status === 409 || err.status === 403 || err.status === 503) {
        try {
          const opKey = matched
            ? matched.opKey || `${matched.method} ${matched.pattern}`
            : `${method} ${pathname}`;
          ...
          /* ignore — the audit emit must never mask the original error */
        }
      }
      try {
        store.save();
      ...
      if (err.code === "unknown_failure_switch" || isApi) {
        ...
        redirect(
          res,
          addFlash(back, null, null, `${err.message} [${err.code}]`),
        );
      } else {
  ...
  function start() {
    ...
    server.listen(config.port, "127.0.0.1", () => {
      console.log(
        `[vwo-002 fixture] ${config.appTitle} (${config.family}) listening on http://localhost:${config.port}/`,
      );
    });
    return server;
  }
