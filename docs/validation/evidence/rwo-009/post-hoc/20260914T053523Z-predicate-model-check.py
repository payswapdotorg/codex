...
def evaluate_upgrade_base(market, workflow):
    ...
    for vid, entry in market.entries.items():
        if (
            ...
            if (
                newest is None
                or entry.metadata["semver"] > market.entries[newest].metadata["semver"]
            ):
                newest = vid
...
def evaluate_upgrade_fixed(market, workflow):
    ...
    for vid, entry in market.entries.items():
        if (
            entry.metadata["workflow"] == workflow
            and vid != install.installed["version_id"]
            and release_installable(
                entry.metadata, entry.state, entry.audience, install.principal
            )
        ):
            if (
                newest is None
...
def decide_upgrade_base(market, proposal, decision):
    ...
    if decision == "Reject":
        market.history.append(
            {"from": proposal["from"], "to": proposal["from"], "applied": False}
        )
        return ("Ok", {"applied": False, "to": proposal["from"]})
    if proposal["to"]["version_id"] not in market.access_allowed:
        return ("Err", "UpgradeRefused")
    market.installs[workflow] = Install(
        install.principal, proposal["to"], install.upgrade_policy
    )
    market.history.append(
        {"from": proposal["from"], "to": proposal["to"]["version_id"], "applied": True}
    )
...
def decide_upgrade_fixed(market, proposal, decision):
    ...
    if proposal["to"]["semver"] < install.installed["semver"]:
        return (
            "Err",
            "IllegalDowngrade",
            install.installed["semver"],
            proposal["to"]["semver"],
        )
    entry = market.entries[proposal["to"]["version_id"]]
    if not release_installable(
        entry.metadata, entry.state, entry.audience, install.principal
    ):
...
def scenario_6d():
    ...
    m.installs["wo"] = Install(
        "bob", {"version_id": "sha256:v1-public-1.0.0", "semver": (1, 0, 0)}, "follow"
    )
...
def scenario_12():
    ...
    m.installs["wo"] = Install(
        "alice", {"version_id": "sha256:v2-public-2.0.0", "semver": (2, 0, 0)}, "follow"
    )
...
print(
    "RWO-009 predicate-logic model check (python transcription of the Rust control flow)"
)
print(
    "base: 2d37cc0ef memory.rs/discovery.rs  |  fixed: rwo-009/upgrade-visibility-gate branch"
)
print(
    "before/after runs share IDENTICAL scenario state (releases, installs, access seeds)"
)
print(
    "NOTE: model of decision logic only — the real crate must still be compiled+tested"
)
print(
    "      (cargo absent in this sandbox; the Tech Lead runs cargo test/clippy independently)"
)
print()

for name, scenario, attack, base_expect, fixed_expect in [
    (
        "ATTACK 6d (Family I, VWO-009 F10)",
        scenario_6d,
        {
            "workflow": "wo",
            "from": "sha256:v1-public-1.0.0",
    ...
    (
        "ATTACK 12 (Family G engine half, VWO-009 F4)",
        scenario_12,
        {
            "workflow": "wo",
            "from": "sha256:v2-public-2.0.0",
    ...
    print(
        ...
        % (
            "None"
            if p_base is None
            else "Some(proposal.to = %s, semver %s)"
    ...
    print(
        "   private identity surfaced to foreign installer? BASE=%s FIXED=%s"
    ...
    for label, m in (("BASE", m_base), ("FIXED", m_fixed)):
        ...
        print(
            "   %s: pin=%s  upgrade_history=%d record(s)  applied=%d"
            % (label, pin, len(m.history), sum(1 for h in m.history if h["applied"]))
        )
    ...
    if name.startswith("ATTACK 6d"):
        ok = (
            ...
            and not leak_fixed
            and r_fixed[0] == "Err"
            and r_fixed[1] == "ReleaseNotVisible"
            and pin_fixed == attack["from"]
            and len(m_fixed.history) == 0
        )
    else:
        ok = (
            r_base[0] == "Ok"
            and r_base[1]["applied"]
            and pin_base == attack["to"]["version_id"]
            and r_fixed[0] == "Err"
            and r_fixed[1] == "IllegalDowngrade"
            and pin_fixed == attack["from"]
            and len(m_fixed.history) == 0
        )
...
print(
    "   stale proposal        -> %s, history=%d  %s"
    % (r, len(m.history), "PASS" if ok1 else "FAIL")
)
...
ok2 = (
    r[0] == "Ok"
    and not r[1]["applied"]
    and r[1]["to"] == "sha256:v2-public-2.0.0"
    and len(m.history) == 1
    and not m.history[0]["applied"]
)
print(
    "   reject decision       -> applied=False to=from, history=1  %s"
    % ("PASS" if ok2 else "FAIL")
)
# legitimate visible forward upgrade (e2e lines 1138-1162): gates pass -> applied
m = Market()
m.entries["sha256:pub-1.0.0"] = Entry(
    {
        "workflow": "wo",
        "semver": (1, 0, 0),
        "ownership": "acme-ops",
        "release": {"version_id": "sha256:pub-1.0.0", "semver": (1, 0, 0)},
    },
    "Public",
    [],
)
m.entries["sha256:pub-1.1.0"] = Entry(
    {
        "workflow": "wo",
        "semver": (1, 1, 0),
        "ownership": "acme-ops",
        "release": {"version_id": "sha256:pub-1.1.0", "semver": (1, 1, 0)},
    },
    "Public",
    [],
)
m.installs["wo"] = Install(
    "alice", {"version_id": "sha256:pub-1.0.0", "semver": (1, 0, 0)}, "follow"
)
...
r = decide_upgrade_fixed(
    ...
    {
        "workflow": "wo",
        "from": "sha256:pub-1.0.0",
        "to": {"version_id": "sha256:pub-1.1.0", "semver": (1, 1, 0)},
    },
    "Approve",
)
ok3 = r[0] == "Ok" and r[1]["applied"] and r[1]["to"] == "sha256:pub-1.1.0"
print(
    "   visible forward 1.0.0 -> 1.1.0 (public) -> applied=True  %s"
...
m.entries["sha256:pub-2.0.0"] = Entry(
    {
        "workflow": "wo",
        "semver": (2, 0, 0),
        "ownership": "acme-ops",
        "release": {"version_id": "sha256:pub-2.0.0", "semver": (2, 0, 0)},
    },
    "Public",
    [],
)
m.installs["wo"] = Install(
    "bob", {"version_id": "sha256:pub-1.0.0", "semver": (1, 0, 0)}, "follow"
)
...
r = decide_upgrade_fixed(
    m,
    {
        "workflow": "wo",
        "from": "sha256:pub-1.0.0",
        "to": {"version_id": "sha256:pub-2.0.0", "semver": (2, 0, 0)},
    },
    "Approve",
)
ok5 = r == ("Err", "UpgradeRefused") and len(m.history) == 0
print(
    "   gate-refused upgrade  -> Err(UpgradeRefused), no record  %s"
...
print(
    "SUMMARY: attack scenarios %s; parity checks %s"
    % (
